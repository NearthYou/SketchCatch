...
test("오른쪽 패널은 final, progress, empty 순서이고 기존 프로젝트에는 progress를 노출하지 않는다", () => {
  ...
  const progressIndex = source.indexOf(") : hasProgressView ? (", finalIndex);
  const emptyIndex = source.indexOf("className={styles.emptyPreview}", progressIndex);

  assert.match(source, /const hasProgressView =\s*existingProject === undefined/);
...
test("첫 snapshot 전 오류나 취소도 progress 화면에서 다시 시도할 수 있다", () => {
  const source = readFileSync(join(currentDir, "workspace-ai-start-client.tsx"), "utf8");

  assert.match(
    source,
    /const hasProgressView =\s*existingProject === undefined &&\s*\(workflow\.progressSnapshot !== null \|\| workflow\.progressStatus !== "idle"\)/
  );
  assert.match(source, /snapshot === null \? "초안 준비"/);
  assert.match(source, /첫 진행 정보를 기다리고 있습니다/);
  assert.match(
    source,
    /workflow\.progressStatus === "interrupted"[\s\S]*workflow\.retryDraft\(\)/
  );
});
