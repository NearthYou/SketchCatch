...
import styles from "../module-thumbnail/module-thumbnail.module.css";

const THUMBNAIL_WIDTH = 1280;
const THUMBNAIL_HEIGHT = 720;

export function TemplateThumbnailCaptureClient({ diagram }: { readonly diagram: DiagramJson }) {
  ...
  return (
    <main className={styles.thumbnailFrame}>
      {captureFailed ? (
        <p data-template-thumbnail-error="true">Template thumbnail capture failed.</p>
      ) : imageDataUrl ? (
        <img
          alt=""
          className={styles.thumbnailImage}
          data-template-thumbnail-ready="true"
...
async function captureBoard(element: HTMLElement): Promise<string> {
  ...
  await Promise.all(Array.from(element.querySelectorAll<HTMLImageElement>("img"), waitForImage));
  ...
  if (!dataUrl.startsWith("data:image/webp;base64,")) {
    throw new Error("Template thumbnail capture did not produce WebP data.");
  }
