...
test("에이전트 리뷰는 확인되지 않은 설정을 노란색 확인 필요로 분류한다", () => {
  const result = createTerraformPreviewPresentation({
    ...preview,
    findings: [],
    wellArchitectedGuidance: [
      {
        observation: "현재 코드에서 네트워크 보안 경계가 보이지 않습니다.",
        pillar: "security",
        recommendation: "보안 그룹의 허용 범위를 확인하세요.",
        title: "보안 에이전트"
      },
      {
        observation: "배포 절차와 리소스 관계가 명확하게 구성되어 있습니다.",
        pillar: "operational_excellence",
        recommendation: "현재 검증 절차를 유지하세요.",
        title: "운영 우수성 에이전트"
      }
    ]
  });
  const severityByLabel = Object.fromEntries(
    result.checks.map((check) => [check.label, check.severity])
  );

  assert.equal(severityByLabel["보안"], "medium");
  assert.equal(severityByLabel["운영"], "low");
  assert.deepEqual(
    result.summaryItems.map((item) => item.label),
    ["잘된 점", "수정해야 할 점"]
  );
  assert.match(result.summaryItems[0]?.text ?? "", /운영:/u);
  assert.doesNotMatch(result.summaryItems[0]?.text ?? "", /유지하세요/u);
  assert.match(result.summaryItems[1]?.text ?? "", /보안 문제:/u);
  assert.match(result.summaryItems[1]?.text ?? "", /보안 수정:/u);
  assert.doesNotMatch(result.summaryItems[1]?.text ?? "", /명확하게 구성/u);
