...
test("에이전트 리뷰는 중간에서 잘린 Amazon Q 문장을 조치나 근거로 표시하지 않는다", () => {
  const result = createTerraformPreviewPresentation({
    ...preview,
    findings: [],
    llmExplanation: {
      fallbackUsed: false,
      highlights: [
        "[보통] 운영 | 판단: 배포 절차가 준비되어 있습니다. | 확인: plan 결과를 검토했습니다.",
        "[확인 필요] 보안 | 판단: 접근 범위를 확인해야 합니다. | 확인: EC2 보안 그룹이.",
        "[확인 필요] 안정성 | 판단: 장애 복구 설정이 부족합니다. | 확인: deployment_circuit_bre.",
        "[보통] 성능 | 판단: Fargate 실행 방식을 사용합니다. | 확인: requires_compatibilities에 FARGATE 명.",
        "[보통] 비용 | 판단: 초기 용량이 작게 설정되어 있습니다. | 확인: 실제 사용량을 확인하세요.",
        "[보통] 지속 가능성 | 판단: 저장소 보호 설정을 검토했습니다. | 확인: encryption과 versioning 설정은 payload에서."
      ],
      nextActions: [],
      summary: "Amazon Q 검토 완료",
      target: "terraform_preview_explanation",
      wellArchitectedConclusion:
        "현재 구성의 장점과 위험을 검토했습니다. 배포 전에 보안과 안정성 설정을 확인해야 합니다. 확인된 근거를 바탕으로 필요한 조치를 적용하세요."
    }
  });

  assert.equal(result.checks[1]?.action, "공개 접근이 필요한지 확인하세요.");
  assert.equal(
    result.checks[2]?.action,
    "장애가 생겼을 때 복구할 방법이 준비되어 있는지 확인하세요."
  );
  assert.equal(
    result.checks[3]?.action,
    "예상 사용량을 처리할 수 있는 크기인지 확인하세요."
  );
  assert.equal(
    result.checks[5]?.action,
    "필요한 만큼만 리소스를 사용하도록 구성했는지 확인하세요."
  );
});

test("에이전트 리뷰는 구분자 없는 판단과 확인도 별도 줄로 분리한다", () => {
  const result = createTerraformPreviewPresentation({
    ...preview,
    findings: [],
    llmExplanation: {
      ...
      highlights: [
        "[보통] 운영 판단: 배포 절차가 준비되어 있습니다. 확인: plan 결과를 검토했습니다.",
        "[확인 필요] 보안 판단: 외부 접근 범위가 넓어 서비스가 노출될 수 있습니다. 확인: EC2 보안 그룹의 인바운드 규칙을 필요한 IP와 포트로 제한하세요.",
        "[보통] 안정성 판단: 복구 절차가 준비되어 있습니다. 확인: 상태 확인 설정을 검토했습니다.",
        "[보통] 성능 판단: 초기 용량이 적절합니다. 확인: 실제 사용량을 확인하세요.",
        "[보통] 비용 판단: 작은 용량으로 시작합니다. 확인: 월별 사용 비용을 확인하세요.",
        "[보통] 지속 가능성 판단: 불필요한 공개 연결을 줄였습니다. 확인: 공개 IP를 할당하지 않았습니다."
      ],
      nextActions: [],
      summary: "Amazon Q 검토 완료",
      target: "terraform_preview_explanation",
      wellArchitectedConclusion:
        "현재 구성의 장점과 위험을 검토했습니다. 배포 전에 보안 설정을 확인해야 합니다. 확인된 근거에 따라 조치하세요."
    }
  });

  assert.equal(
    result.checks[1]?.summary,
    "외부 접근 범위가 넓어 서비스가 노출될 수 있습니다."
  );
  assert.equal(
    result.checks[1]?.action,
    "EC2 보안 그룹의 인바운드 규칙을 필요한 IP와 포트로 제한하세요."
  );
});
...
test("오류 분석은 오류 유형과 위치를 제목으로 표시하고 원문은 기술 정보에 둔다", () => {
...
test("에이전트 리뷰의 Terraform 리소스 이름을 중복 없는 한국어로 바꾼다", () => {
  const result = createTerraformPreviewPresentation({
    ...preview,
    findings: [],
    wellArchitectedGuidance: [
      {
        observation: "aws_s3_bucket 리소스의 암호화 설정을 확인해야 합니다.",
        pillar: "security",
        recommendation: "resource.storage.aws_s3_bucket 리소스에 암호화를 선언하세요.",
        title: "보안 설정 확인"
      }
    ]
  });
  const visibleResult = JSON.stringify(result.checks);

  assert.match(visibleResult, /해당 리소스/u);
  assert.doesNotMatch(visibleResult, /해당 리소스 리소스/u);
});
