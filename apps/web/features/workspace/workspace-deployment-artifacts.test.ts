...
test("saveWorkspaceTerraformArtifact preserves multiple Terraform files in one bundle", async (context) => {
  const originalFetch = globalThis.fetch;
  const originalWindowDescriptor = Object.getOwnPropertyDescriptor(globalThis, "window");
  const requests: Array<{ input: RequestInfo | URL; init?: RequestInit | undefined }> = [];
  ...
  globalThis.fetch = async (input, init) => {
      ...
      return jsonResponse(
        {
          asset: {
            ...
            architectureId: "55555555-5555-4555-8555-555555555555",
            assetType: "terraform_file",
            objectKey: "projects/project/assets/terraform_file/terraform-files.json",
            fileName: "terraform-files.json",
            contentType: "application/vnd.sketchcatch.terraform-files+json",
            byteSize: new TextEncoder().encode(bundle).byteLength,
            uploadStatus: "pending",
            createdAt: "2026-06-26T00:00:00.000Z"
          },
          upload: {
            ...
            url: "https://s3.example.test/upload",
            headers: {
              "Content-Type": "application/vnd.sketchcatch.terraform-files+json"
            },
            expiresInSeconds: 900
          }
        },
    ...
    if (String(input).includes("confirm-upload")) {
      return jsonResponse({
        asset: {
          id: "66666666-6666-4666-8666-666666666666",
          projectId: project.id,
  ...
  await saveWorkspaceTerraformArtifact({
    diagramJson,
    projectId: project.id,
    terraformCode: terraformFiles.map((file) => file.terraformCode).join("\n"),
    terraformFiles
  });
  ...
  assert.equal(requests[3]?.init?.body, bundle);
});
