...
test("현재 상태와 사용자 영향을 먼저 보여주고 중요 신호는 세 개 이하로 렌더링한다", () => {
  ...
  assertAppearsInOrder(text, ["현재 상태", "중요한 문제", "문제 상세"]);
...
test("관측 전에는 확인 중 상태와 안내만 보여주고 문제를 만들어내지 않는다", () => {
  ...
  assert.match(html, /상태를 확인하고 있어요\./);
  assert.match(html, /최신 상태가 여기에 표시돼요/);
  assert.doesNotMatch(html, /관측 데이터가 아직 없어요/);
...
test("수정안 진행 상태는 AI나 원본 오류를 사용자에게 노출하지 않는다", () => {
  ...
  assert.doesNotMatch(text, /AI|Provider|snapshot|max_capacity|Project Draft|Design Simulation|Resource/);
});
