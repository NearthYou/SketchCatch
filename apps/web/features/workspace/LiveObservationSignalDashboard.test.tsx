...
test("CloudWatch 지표를 먼저 보여주고 관측 기록은 세 개 이하로 유지한다", () => {
  ...
  assert.ok(html.indexOf("CloudWatch 기반 지표") < signalHeadingIndex);
  assert.equal((html.match(/aria-pressed=/g) ?? []).length, 3);
  const text = renderedText(html);
  assertAppearsInOrder(text, ["CloudWatch 기반 지표", "관측 기록", "문제 상세"]);
  assert.doesNotMatch(text, /현재 상태|지금 확인할 내용|선택한 신호|다음 확인/);
  assert.doesNotMatch(text, /관련 로그를 확인해 보세요/);
  assert.match(html, /인프라 전체 평가/);
  assert.match(html, /CloudWatch 관측 근거/);
  assert.match(html, /병목 예상 지점/);
  assert.match(html, /ALB 대상 그룹에서 ECS Task로 요청을 전달하는 구간/);
  assert.doesNotMatch(html, /가능성이 높은 원인|아직 확인할 수 없는 부분/);
...
test("문제 상세에서 시간순 기록 disclosure를 만들지 않는다", () => {
  ...
  assert.match(html, /인프라 전체 평가/);
  assert.doesNotMatch(html, /아직 확인할 수 없는 부분/);
});
...
test("관측 전에는 수집 지표의 대기 상태만 보여주고 문제를 만들어내지 않는다", () => {
  ...
  assert.match(text, /CloudWatch 기반 지표/);
  assert.match(text, /요청량 0 req\/min/);
  assert.match(text, /실행 확인 중 · 예상 계산 중/);
  assert.doesNotMatch(text, /설계 분석|분석 완료/);
...
test("참여 요청이 도착하면 현재 상태 카드 없이 수집 건수를 갱신한다", () => {
  ...
  assert.match(text, /CloudWatch 기반 지표/);
  assert.match(text, /요청량 12 req\/min/);
  assert.doesNotMatch(text, /현재 상태|상태를 확인하고 있어요/);
});
