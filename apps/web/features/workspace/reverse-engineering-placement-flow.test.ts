...
test("보드 적용은 Terraform 선택이나 확인 없이 빈 observe-only 결정을 서버에 보낸다", () => {
  const applyFlow = getSourceBlock(
    panelSource,
    "async function applyScanResult(",
    "function previewApplicationMode("
  ...
  assert.doesNotMatch(applyFlow, /isImportDecisionComplete/);
  assert.doesNotMatch(applyFlow, /selectedReadyResourceIds/);
  assert.doesNotMatch(applyFlow, /acknowledgedReviewOnlyResourceIds/);
  assert.match(
    applyFlow,
    /const importDecision = createObserveOnlyReverseEngineeringImportDecision\(\);/
  );
  assert.match(panelSource, /selectedReadyResourceIds: \[\]/);
  assert.match(panelSource, /acknowledgedReviewOnlyResourceIds: \[\]/);
