...
test("VPC는 배포 보장 대신 Terraform 편집 대상이라고 정확히 표시한다", () => {
  ...
  assert.equal(presentation.statusLabel, "Terraform 편집 대상");
  assert.equal(
    presentation.statusDescription,
    "AWS에서 가져온 설정을 Terraform으로 확인하고 수정할 수 있습니다. 실제 배포 전에는 변경 계획을 검토해야 합니다."
  );
...
test("필수 정보가 부족한 Resource는 무엇을 해야 하는지 바로 표시한다", () => {
  ...
  assert.equal(presentation.statusLabel, "설정 보완 필요");
  assert.equal(
    presentation.statusDescription,
    "AWS에서 읽은 정보가 부족하거나 연결 설정을 자동으로 옮길 수 없습니다. 표시된 항목을 확인한 뒤 Terraform에 포함하세요."
  );
});
