...
const GENERIC_ROLE_PATTERNS: ProviderLayoutMapping["rolePatterns"] = [
  { pattern: /\b(security|identity|permission|policy|role|secret|certificate|encryption)\b/u, role: "security" },
  { pattern: /\b(observability|monitor|metric|alarm|logging|logs?|tracing|telemetry)\b/u, role: "observability" },
  { pattern: /\b(ci\/?cd|pipeline|delivery|deployment|registry|build|release)\b/u, role: "delivery" },
  ...
  { pattern: /\b(network|region|zone|subnet|route|firewall|vpc|virtual network)\b/u, role: "network" }
];
...
const AWS_LAYOUT_MAPPING: ProviderLayoutMapping = {
  ...
  rolePatterns: [
    { pattern: /iam|kms|cognito|guardduty|shield|waf|acm|secretsmanager|security_group/u, role: "security" },
    { pattern: /cloudwatch|xray/u, role: "observability" },
    { pattern: /codebuild|codepipeline|codedeploy|ecr/u, role: "delivery" },
    { pattern: /cloudfront|route53|api_gateway|apigateway|load_balancer|\baws_lb\b/u, role: "entry" },
...
export function getAutomaticDiagramSemanticRole(node: DiagramNode): AutomaticDiagramSemanticRole {
  ...
  const providerMapping = PROVIDER_LAYOUT_MAPPINGS.find((mapping) => mapping.matchesResourceType(resourceType));
