...
test("increments the single ECS Application Auto Scaling max_capacity by one", () => {
  const result = incrementLiveObservationEcsScalingSettings([
    {
      fileName: "main.tf",
      terraformCode: [
        ...
        "}",
        "",
        'resource "aws_appautoscaling_policy" "audience_requests" {',
        '  policy_type = "TargetTrackingScaling"',
        '  service_namespace = "ecs"',
        '  scalable_dimension = "ecs:service:DesiredCount"',
        '  resource_id = "service/${aws_ecs_cluster.audience.name}/${aws_ecs_service.audience.name}"',
        "  target_tracking_scaling_policy_configuration {",
        "    target_value = 60 # requests per task",
  ...
  assert.equal(result.previousTargetValue, 60);
  assert.equal(result.nextTargetValue, 61);
  ...
  assert.match(result.files[0]?.terraformCode ?? "", /target_value = 61 # requests per task/);
...
test("refuses to update scaling settings for different ECS services", () => {
  assert.throws(
    () =>
      incrementLiveObservationEcsScalingSettings([
        {
          ...
          terraformCode: [
            ...
            '  service_namespace = "ecs"',
            '  scalable_dimension = "ecs:service:DesiredCount"',
            '  resource_id = "service/demo/other"',
            "  target_tracking_scaling_policy_configuration {",
            "    target_value = 60",
            "    predefined_metric_specification {",
            '      predefined_metric_type = "ALBRequestCountPerTarget"',
