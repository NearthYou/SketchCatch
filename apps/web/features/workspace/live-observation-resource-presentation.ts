...
const RESOURCE_ROLE_LABELS: Readonly<Record<string, string>> = {
  ...
  LOAD_BALANCER_TARGET_GROUP: "서버 연결",
  RDS_CLUSTER: "데이터베이스",
  RDS_INSTANCE: "데이터베이스",
  S3: "웹 파일 저장소",
  ...
  aws_lb_target_group: "서버 연결",
  aws_db_instance: "데이터베이스",
  aws_rds_cluster: "데이터베이스",
...
const GENERIC_AWS_LABELS = new Set([
  "alb",
  "cloudfront",
...
export function getLiveObservationResourceDisplayName(node: DiagramNode): string {
  ...
  if (!shouldReplaceResourceLabel(label, node)) return label;

  const resourceType = node.parameters?.resourceType ?? node.type;
  return RESOURCE_ROLE_LABELS[resourceType] ?? "AWS 리소스";
}
...
function createDisplayNamesByNodeId(nodes: readonly DiagramNode[]): ReadonlyMap<string, string> {
  ...
  for (const node of nodes) {
    if (node.kind !== "resource" || !shouldReplaceResourceLabel(node.label.trim(), node)) {
...
function shouldReplaceResourceLabel(label: string, node: DiagramNode): boolean {
  if (isInternalResourceLabel(label, node)) return true;

  const resourceType = node.parameters?.resourceType ?? node.type;
  if (!RESOURCE_ROLE_LABELS[resourceType]) return false;
  const normalizedLabel = label.toLocaleLowerCase("en-US");
  const catalogLabel = CATALOG_LABELS_BY_RESOURCE_TYPE.get(resourceType)?.toLocaleLowerCase("en-US");
  return normalizedLabel === catalogLabel || GENERIC_AWS_LABELS.has(normalizedLabel);
}
