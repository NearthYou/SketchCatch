...
test("UNKNOWN 원본도 이미 아는 AWS provider type이면 가장 가까운 팔레트 아이콘으로 표시한다", () => {
  const providerResourceTypes = reverseEngineeringAwsResourceCatalog.flatMap(
    (entry) => entry.providerResourceTypes
  );
  const architecture: ArchitectureJson = {
    nodes: providerResourceTypes.map((providerResourceType, index) => ({
      id: `known-provider-${index}`,
      type: "UNKNOWN",
      positionX: index * 100,
      positionY: 80,
      config: {
  ...
  const resources = createSourceExactReverseEngineeringDiagram(architecture).nodes.filter(
    (node) => node.kind === "resource"
  );

  assert.equal(resources.length, providerResourceTypes.length);
  for (const [index, resource] of resources.entries()) {
    const providerResourceType = providerResourceTypes[index] ?? "(missing provider resource type)";

    assert.equal(resource.type, "UNKNOWN");
    assert.equal(resource.parameters?.invalid, true);
    assert.equal(resource.parameters?.resourceType, "");
    assert.equal(resource.parameters?.values["providerResourceType"], providerResourceType);
    assert.notEqual(resource.label, "기타 AWS 리소스", providerResourceType);
    assert.match(resource.iconUrl ?? "", /\.svg$/u, providerResourceType);
    assert.doesNotMatch(
      resource.iconUrl ?? "",
      /AWS-Management-Console_48_Light\.svg$/u,
      providerResourceType
    );
  }
});
...
test("새 AWS 종류가 아직 전용 팔레트에 없어도 기타 AWS 리소스로 숨기지 않는다", () => {
  ...
  assert.equal(resource?.type, "UNKNOWN");
  assert.equal(resource?.label, "CloudHSM Cluster");
  assert.equal(resource?.parameters?.invalid, true);
