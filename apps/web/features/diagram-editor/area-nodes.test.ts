...
test("area node helpers distinguish design containers from resource containers", () => {
  ...
  const resourceRegion = makeResourceNode({ resourceType: "aws_region" });
  ...
  assert.equal(isDesignAreaNode(resourceRegion), false);
  assert.equal(isResourceAreaNode(resourceRegion), true);
