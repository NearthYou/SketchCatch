import {
  BRAINBOARD_TEMPLATE_IDS as REFERENCE_TEMPLATE_IDS,
  brainboardFailedCaptureEvidence as unavailableReferenceTemplateEvidence,
  type BrainboardTemplateId as ReferenceTemplateId
} from "../../../../packages/types/src";
import { BOARD_THUMBNAIL_CAPTURE_CONTRACT } from "../../components/architecture-board/board-thumbnail-capture-contract";

export type ReferenceBoardCaptureAsset = {
  readonly captureVersion: number;
  readonly diagramHash: string;
  readonly kind: "board-capture";
  readonly src: string;
  readonly templateId: ReferenceTemplateId;
};

export type ReferenceSourcePreviewAsset = {
...
export type ReferenceTemplateThumbnailAsset =
  | ReferenceBoardCaptureAsset
  | ReferenceSourcePreviewAsset;

const REFERENCE_THUMBNAIL_VERSION = `v${BOARD_THUMBNAIL_CAPTURE_CONTRACT.version}`;

const REFERENCE_DIAGRAM_HASHES: Record<
  Exclude<ReferenceTemplateId, "brainboard-aws-instance-db-multiple-networks">,
  string
> = {
...
export const REFERENCE_TEMPLATE_THUMBNAIL_ASSETS = Object.fromEntries(
  REFERENCE_TEMPLATE_IDS.map(
    (templateId): readonly [ReferenceTemplateId, ReferenceTemplateThumbnailAsset] => {
      const fileName = templateId.slice(templateId.indexOf("-") + 1);
      const src = `/template-thumbnails/reference/${REFERENCE_THUMBNAIL_VERSION}/${fileName}.webp`;

      if (templateId === unavailableReferenceTemplateEvidence.id) {
        return [
          templateId,
          {
            kind: "source-preview",
            originalHeight: unavailableReferenceTemplateEvidence.origin.previewHeight,
            originalWidth: unavailableReferenceTemplateEvidence.origin.previewWidth,
            sourcePreviewUrl: unavailableReferenceTemplateEvidence.origin.previewUrl,
            src,
            templateId
          }
        ];
      ...
      return [
        templateId,
        {
          ...
          diagramHash: REFERENCE_DIAGRAM_HASHES[templateId],
          kind: "board-capture",
          src,
          templateId
        }
...
) as Record<ReferenceTemplateId, ReferenceTemplateThumbnailAsset>;

export function getReferenceTemplateThumbnailAsset(
  templateId: ReferenceTemplateId
): ReferenceTemplateThumbnailAsset {
  return REFERENCE_TEMPLATE_THUMBNAIL_ASSETS[templateId];
}
