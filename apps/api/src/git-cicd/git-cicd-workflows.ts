...
export type GitCicdWorkflowRenderInput = {
  ...
  sketchCatchPublicBaseUrl?: string | null | undefined;
...
function renderEcsFargateAppWorkflow(
  ...
  const environmentName = input.environmentName ?? defaultGitCicdEnvironmentName;
  const appPathGlob = createMonitoredPathGlob(
    input.appPath ?? ecs.confirmedBuildConfig.sourceRoot
  );
  const generatedProjectPath = `sketchcatch/${input.projectSlug}`;
...
jobs:
  release:
    ...
    steps:
      - name: Request trusted SketchCatch release
        ...
        run: |
          ...
          jq -n \
            ...
            --arg commitSha "$SKETCHCATCH_RELEASE_SHA" \
            --arg ref "\${{ github.ref }}" \
          ...
          curl --fail --silent --show-error --retry 5 --retry-all-errors --retry-delay 2 --connect-timeout 10 --max-time 30 \
            ...
            "\${SKETCHCATCH_RELEASE_API_URL%/}/api/git-cicd/projects/$SKETCHCATCH_PROJECT_ID/release-runs" > release-run.json
      ...
      - name: Wait for trusted release
        shell: bash
        env:
          SKETCHCATCH_RELEASE_RUN_ID: \${{ steps.release.outputs.run_id }}
        run: |
          set -euo pipefail
          for attempt in $(seq 1 180); do
            ...
            echo "SketchCatch release status: $STATUS"
            case "$STATUS" in
              succeeded) exit 0 ;;
              failed|cancelled|partially_failed|partially_cancelled) jq . release-run.json; exit 1 ;;
            esac
            sleep 5
          done
          echo "SketchCatch release status polling timed out" >&2
          exit 1
`;
}
  ...
  deploy:
    ...
    steps:
      - name: Register SketchCatch infrastructure run
        ...
        run: |
          ...
          curl --fail --silent --show-error \
            ...
            --header "Authorization: Bearer $OIDC_TOKEN" \
            --header "Content-Type: application/json" \
            --data @infrastructure-run-request.json \
            "\${SKETCHCATCH_RELEASE_API_URL%/}/api/git-cicd/projects/$SKETCHCATCH_PROJECT_ID/infrastructure-runs" > infrastructure-run.json
      ...
      - name: Prepare Terraform
        id: prepare
        if: steps.checkout.outcome == 'success' && steps.terraform.outcome == 'success' && steps.aws.outcome == 'success'
        continue-on-error: true
        shell: bash
        env:
          SKETCHCATCH_INFRA_RUN_ID: \${{ steps.register.outputs.run_id }}
        run: |
          set -euo pipefail
          request_oidc_token() {
      ...
      - uses: actions/upload-artifact@v4
        ...
        with:
          name: sketchcatch-tfplan-json
          path: ${terraformDirectory}/tfplan.json
      - name: Terraform Apply
        ...
        run: |
          ...
          send_heartbeat
          run_with_heartbeat terraform apply -auto-approve tfplan
          send_heartbeat
          terraform output -json > sketchcatch-outputs.json
      - uses: actions/upload-artifact@v4
        if: steps.apply.outcome == 'success'
        with:
          name: sketchcatch-terraform-outputs
          path: ${terraformDirectory}/sketchcatch-outputs.json
      - name: Report SketchCatch infrastructure result
        if: always() && steps.register.outputs.run_id != ''
        shell: bash
        working-directory: .
        env:
          SKETCHCATCH_INFRA_RUN_ID: \${{ steps.register.outputs.run_id }}
        run: |
          ...
          if [ "\${{ job.status }}" = "cancelled" ]; then
            ...
            STAGE=configuration
               ...
               [ "\${{ steps.terraform.outcome }}" != "success" ] || \
               [ "\${{ steps.aws.outcome }}" != "success" ] || \
               [ "\${{ steps.prepare.outcome }}" != "success" ]; then
            CONCLUSION=failed
            STAGE=configuration
          ...
          curl --fail --silent --show-error \
            ...
            --header "Authorization: Bearer $OIDC_TOKEN" \
            --header "Content-Type: application/json" \
            --data @infrastructure-run-result.json \
            "\${SKETCHCATCH_RELEASE_API_URL%/}/api/git-cicd/infrastructure-runs/$SKETCHCATCH_INFRA_RUN_ID/complete" >/dev/null
