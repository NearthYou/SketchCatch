...
test("KMS Grant를 모든 page에서 발견하면 민감한 원문 없이 Key와 Alias를 관리 불가로 닫는다", async () => {
  const grantSecrets = [
    "arn:aws:iam::123456789012:role/private-grantee",
    "Decrypt-private-operation",
    "private-grant-token"
  ];
  const grantMarkers: Array<string | undefined> = [];
  const result = await readKmsResources({
    region: "ap-northeast-2",
    client: {
      async send(command: object): Promise<unknown> {
        if (command instanceof ListAliasesCommand) {
          return {
            Aliases: [{ AliasName: "alias/orders", TargetKeyId: "key-granted" }],
            Truncated: false
          };
        }
        if (command instanceof ListKeysCommand) {
          return { Keys: [{ KeyId: "key-granted" }], Truncated: false };
        }
        if (command instanceof DescribeKeyCommand) {
          return {
            KeyMetadata: {
        ...
        if (command instanceof ListGrantsCommand) {
          grantMarkers.push(command.input.Marker);
          return command.input.Marker === undefined
            ? {
                Grants: [
                  {
                    GranteePrincipal: grantSecrets[0],
                    Operations: [grantSecrets[1]],
                    GrantToken: grantSecrets[2]
                  }
                ],
                Truncated: true,
                NextMarker: "next-grants"
              }
            : {
                Grants: [{ RetiringPrincipal: "arn:aws:iam::123456789012:role/private-retiring" }],
                Truncated: false
              };
        }
  ...
  assert.deepEqual(grantMarkers, [undefined, "next-grants"]);
  assert.equal(result.failures.length, 0);
  assert.equal(key?.config["hasGrants"], true);
  assert.equal(key?.config["grantCount"], 2);
  assert.equal(key?.config["grantsReadComplete"], true);
  assert.equal(key?.config["managementReady"], false);
  assert.equal(key?.config["reverseEngineeringDetailsComplete"], false);
  assert.equal(alias?.config["managementReady"], false);
  assert.equal(alias?.config["reverseEngineeringDetailsComplete"], false);
  for (const secret of grantSecrets) {
    assert.doesNotMatch(JSON.stringify(result), new RegExp(secret, "u"));
  }
  assert.doesNotMatch(JSON.stringify(result), /private-retiring/u);
});

test("KMS Grant pagination이 반복되거나 조회가 실패하면 Key와 Alias를 완료로 표시하지 않는다", async () => {
  for (const scenario of ["repeated_marker", "read_failure"] as const) {
    let grantCalls = 0;
    const result = await readKmsResources({
      region: "ap-northeast-2",
      client: {
        async send(command: object): Promise<unknown> {
          if (command instanceof ListAliasesCommand) {
            return {
              Aliases: [{ AliasName: "alias/orders", TargetKeyId: `key-${scenario}` }],
              Truncated: false
            };
          }
          if (command instanceof ListKeysCommand) {
            return { Keys: [{ KeyId: `key-${scenario}` }], Truncated: false };
          }
          if (command instanceof DescribeKeyCommand) {
            return {
              KeyMetadata: {
          ...
          if (command instanceof ListGrantsCommand) {
            ...
            if (scenario === "read_failure") {
              throw new Error("AccessDenied private grant detail");
            }
            return { Grants: [], Truncated: true, NextMarker: "same-marker" };
          }
          if (command instanceof ListResourceTagsCommand) return { Tags: [], Truncated: false };
          throw new Error(`unexpected ${command.constructor.name}`);
        }
    ...
    const key = result.records.find((record) => record.providerResourceType === "AWS::KMS::Key");
    const alias = result.records.find(
      (record) => record.providerResourceType === "AWS::KMS::Alias"
    );

    assert.equal(grantCalls, scenario === "repeated_marker" ? 2 : 1);
    assert.equal(
      result.failures.some(
        (failure) =>
          failure.operation ===
          (scenario === "repeated_marker" ? "ListGrantsPagination" : "ListGrants")
      ),
      true
    );
    assert.equal(key?.config["grantsReadComplete"], false);
    assert.equal(key?.config["grantCount"], undefined);
    assert.equal(key?.config["hasGrants"], undefined);
    assert.equal(key?.config["managementReady"], false);
    assert.equal(key?.config["reverseEngineeringDetailsComplete"], false);
    assert.equal(alias?.config["managementReady"], false);
    assert.equal(alias?.config["reverseEngineeringDetailsComplete"], false);
    assert.doesNotMatch(JSON.stringify(result), /private grant detail/u);
  }
});

test("KMS custom rotation period를 보존하고 활성 회전 주기가 빠지면 관리 가능으로 표시하지 않는다", async () => {
  const read = async (rotationResponse: Record<string, unknown>) =>
    readKmsResources({
      region: "ap-northeast-2",
      client: {
        async send(command: object): Promise<unknown> {
          ...
          if (command instanceof ListKeysCommand) {
            return { Keys: [{ KeyId: "key-rotation" }], Truncated: false };
          }
          if (command instanceof DescribeKeyCommand) {
            return {
              KeyMetadata: {
          ...
          if (command instanceof GetKeyPolicyCommand) return { Policy: "{}" };
          if (command instanceof GetKeyRotationStatusCommand) return rotationResponse;
          if (command instanceof ListGrantsCommand) return { Grants: [], Truncated: false };
          if (command instanceof ListResourceTagsCommand) return { Tags: [], Truncated: false };
          throw new Error(`unexpected ${command.constructor.name}`);
        }
      }
  ...
  const customPeriod = await read({ KeyRotationEnabled: true, RotationPeriodInDays: 180 });
  const missingPeriod = await read({ KeyRotationEnabled: true });
  const customKey = customPeriod.records.find(
    (record) => record.providerResourceType === "AWS::KMS::Key"
  );
  const incompleteKey = missingPeriod.records.find(
    (record) => record.providerResourceType === "AWS::KMS::Key"
  );

  assert.equal(customKey?.config["rotationPeriodInDays"], 180);
  assert.equal(customKey?.config["rotationReadComplete"], true);
  assert.equal(customKey?.config["managementReady"], true);
  assert.equal(incompleteKey?.config["rotationPeriodInDays"], undefined);
  assert.equal(incompleteKey?.config["rotationReadComplete"], false);
  assert.equal(incompleteKey?.config["managementReady"], false);
});

test("Alias와 설명이 없는 KMS Key의 exact Key ID를 공개 displayName으로 사용하지 않는다", async () => {
  const exactKeyId = "1234abcd-12ab-34cd-56ef-1234567890ab";
  const result = await readKmsResources({
    region: "ap-northeast-2",
    client: {
      async send(command: object): Promise<unknown> {
        if (command instanceof ListAliasesCommand) return { Aliases: [], Truncated: false };
        if (command instanceof ListKeysCommand) {
          return { Keys: [{ KeyId: exactKeyId }], Truncated: false };
        }
        if (command instanceof DescribeKeyCommand) {
          return {
            KeyMetadata: {
  ...
  assert.doesNotMatch(key?.displayName ?? "", new RegExp(exactKeyId, "u"));
  assert.match(key?.displayName ?? "", /^KMS Key · [a-f0-9]{7}$/u);
  assert.equal(
    result.serverOnlyDetails.find((detail) => detail.resourceKind === "key")?.terraformImportId,
    exactKeyId
  );
});
...
test("KMS pagination 응답의 완료 표시와 marker가 모순이면 실패로 닫는다", async () => {
  const contradictoryScopes = ["keys", "aliases", "tags"] as const;

  for (const scope of contradictoryScopes) {
    const client = {
      async send(command: object): Promise<unknown> {
        if (command instanceof ListAliasesCommand) {
          return scope === "aliases"
        ...
        if (command instanceof ListResourceTagsCommand) {
          return scope === "tags"
            ? { Tags: [], Truncated: false, NextMarker: "unexpected" }
    ...
    assert.equal(
      result.failures.some((failure) => failure.operation === expectedOperation),
      true,
      `${scope} contradiction must record a pagination failure`
    );
    assert.equal(
      result.records.find((record) => record.providerResourceType === "AWS::KMS::Key")?.config[
        "managementReady"
...
test("빈 KMS policy와 불완전한 tag는 완료로 처리하지 않고 원문을 서버 결과에 남기지 않는다", async () => {
  const malformedScopes = ["policy", "tag"] as const;

  for (const scope of malformedScopes) {
    const exactKeyId = `key-${scope}`;
    const result = await readKmsResources({
      region: "ap-northeast-2",
      client: {
        async send(command: object): Promise<unknown> {
    ...
    const key = result.records.find((record) => record.providerResourceType === "AWS::KMS::Key");
    const privateKey = result.serverOnlyDetails.find((detail) => detail.resourceKind === "key");

    assert.equal(key?.config["managementReady"], false);
    assert.equal(key?.config["reverseEngineeringDetailsComplete"], false);
    if (scope === "policy") {
      assert.equal(key?.config["policyReadComplete"], false);
      assert.equal(privateKey && "policyDocument" in privateKey, false);
    } else {
      assert.equal(key?.config["tagsReadComplete"], false);
      assert.equal(privateKey && "tags" in privateKey, false);
    }
  }
});
