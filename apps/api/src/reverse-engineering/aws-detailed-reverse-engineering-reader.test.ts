  ...
  const result = await readAwsDetailedReverseEngineeringResources(
    {
      provider: "aws",
      region: "ap-northeast-2",
      // UI에서 Stage·Method 등 하위 선택은 이 parent scan 값으로 전송된다.
      resourceTypes: ["API_GATEWAY_REST_API"]
    },
    CREDENTIALS,
    createDependencies([])
...
test("UI가 KMS Alias 선택을 Key로 정규화해도 Alias와 Key 관계를 남긴다", async () => {
  const dependencies = createDependencies([]);
  dependencies.readKms = async () => ({
    records: [
      record("AWS::KMS::Key", "kms-key-ref", "orders-key"),
      {
        ...record("AWS::KMS::Alias", "alias/orders", "alias/orders"),
        relationships: [{ type: "depends_on", targetProviderResourceId: "kms-key-ref" }]
      }
    ],
    serverOnlyDetails: [],
    failures: []
  ...
  const result = await readAwsDetailedReverseEngineeringResources(
    {
      provider: "aws",
      region: "ap-northeast-2",
      // UI에서 KMS_ALIAS는 KMS_KEY scan 값으로 전송된다.
  ...
  assert.deepEqual(
    result.records.find((record) => record.providerResourceType === "AWS::KMS::Alias")
      ?.relationships,
    [{ type: "depends_on", targetProviderResourceId: "kms-key-ref" }]
  );
});
