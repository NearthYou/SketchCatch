...
test("Cloud Control 목록 제한은 발견된 AWS 종류에만 중립적으로 남긴다", async () => {
  ...
  assert.deepEqual(result.scanErrors[0]?.affectedProviderResourceTypes, [
    "AWS::CertificateManager::Certificate",
    "AWS::CertificateManager::CertificateValidation"
  ]);
  assert.doesNotMatch(JSON.stringify(result), /private handler error/iu);
});
...
test("가까운 팔레트 아이콘이 있어도 정확하지 않은 AWS 종류는 Terraform 대상이 되지 않는다", async () => {
  ...
  assert.match(
    result.importSuggestions[1]?.reason ?? "",
    /AWS::EC2::SecurityGroupRule.*기타 AWS 리소스/u
  );
  assert.match(
    result.analysisExclusions[1]?.message ?? "",
    /AWS::EC2::SecurityGroupRule.*원본 종류·리전·관계/u
  );
  assert.deepEqual(
