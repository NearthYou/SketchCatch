...
test("관계가 부족한 Target Group과 Listener는 실제 누락 연결을 안내한다", async () => {
  ...
  assert.equal(
    targetSuggestion?.reason,
    "AWS에서 찾았지만 안전하게 수정할 설정을 더 확인해야 합니다."
  );
  assert.equal(targetExclusion?.reason, "missing_required_data");
  ...
  assert.equal(
    listenerSuggestion?.reason,
    "AWS에서 찾았지만 안전하게 수정할 설정을 더 확인해야 합니다."
  );
  assert.equal(listenerExclusion?.reason, "missing_required_data");
