...
test("publisher base URLs match the pinned signer for virtual-hosted and dotted buckets", async () => {
  ...
  for (const bucketName of [templateInput.bucketName, "sketchcatch.private.templates"]) {
    ...
    assert(
      getAwsImportPresignedTemplateUrlPatterns(expectedBaseUrl).every((pattern) =>
