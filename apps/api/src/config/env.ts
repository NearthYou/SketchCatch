...
export function getRuntimeEnv(): RuntimeEnv {
  return {
    ...
    githubAppCallbackUrl: process.env.GITHUB_APP_CALLBACK_URL,
    githubAppStateSecret: process.env.GITHUB_APP_STATE_SECRET,
...
export function requireGitHubAppConfig(): {
  ...
  if (!privateKeyBase64) {
    throw new Error("GITHUB_APP_PRIVATE_KEY_BASE64 is required");
  }

  if (!callbackUrl) {
    throw new Error("GITHUB_APP_CALLBACK_URL is required");
  }
...
export function requireGitHubAppStateSecret(): string {
  const stateSecret = process.env.GITHUB_APP_STATE_SECRET?.trim() || requireAuthTokenSecret();

  if (stateSecret.length < 32) {
    throw new Error("GITHUB_APP_STATE_SECRET must be at least 32 characters");
  }
