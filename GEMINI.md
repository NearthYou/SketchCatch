...
이 파일은 Gemini Code Assist가 SketchCatch 저장소에서 작업할 때 따라야 하는 지침입니다.

## 응답 언어

1. 모든 설명, 리뷰, 요약, PR/Issue 초안은 한글로만 작성합니다.
2. 코드, 명령어, 파일명, 패키지명, API 이름처럼 원문 표기가 필요한 경우에만 영어를 사용합니다.
3. 사용자가 다른 언어를 명시적으로 요청하지 않는 한 한글 응답 원칙을 유지합니다.

## 프로젝트 기준

1. 이 저장소는 SketchCatch입니다.
2. 현재 목표는 AWS 초보자를 위한 안전한 학습 환경을 만드는 것입니다.
3. 명시적으로 요청받기 전까지 실제 AWS 배포, Terraform 실행, AWS SDK 연동, AI SDK 연동, 인증, ORM, 데이터베이스 마이그레이션을 구현하지 않습니다.
4. Secret, 실제 클라우드 자격 증명, `.env` 파일을 커밋하지 않습니다.
...
작업을 마치기 전에 다음 명령을 실행하고 실패 시 실패 내용을 정확히 보고합니다.
