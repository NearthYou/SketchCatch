...
## 2026-07-10 ECS Phase 3-5 progress compacted after dev merge

- Archived the detailed ECS runtime secret transition, deployment job model, and API worker dispatch session records from `agent-progress.md` after merging `dev` into `feat/ck/287-ai-diagram`.
- Phase 3 replaced generated ECS env-file dependence with validated task-definition environment and secret references while preserving EC2 rollback.
- Phase 4 added the internal `DeploymentJob` model, migrations, lifecycle helpers, and focused service coverage.
- Phase 5 added opt-in ECS RunTask dispatch, cancellation support, route/config wiring, and focused dispatcher coverage; the worker runtime remained deferred to Phase 6.
- Verification recorded by those sessions included `pnpm harness:check`, `pnpm lint`, `pnpm typecheck`, `pnpm build`, focused API tests, Terraform format/validate, and static workflow checks.
- No archived phase ran live AWS mutation, Terraform apply/destroy, or production deployment.
