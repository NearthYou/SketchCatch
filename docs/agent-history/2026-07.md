...
- Added separate estimated-cost and actual-usage panels, project scoping, direct expected-user input, and refresh feedback.
- Verification: focused API and Web tests, lint, typecheck, build, and harness passed.

### 2026-07-13 - Repair Terraform nested-block merge regression

- Consolidated duplicate Launch Template nested-block definitions while preserving IAM profile, metadata, monitoring, network interface, and tag support.
- Verification: full typecheck, focused Terraform regressions, lint, build, harness, and diff checks passed.

### 2026-07-13 - Add and review diagram-based Live Observation for ECS Fargate and ASG

- Added diagram-derived traffic paths, polling-compatible snapshots, CloudWatch Agent/ASG and ECS Fargate observation, and capacity visualization.
- Added disconnect-safe SSE startup, expiring simulated traffic, metric-correct thresholds, Traffic API audience links, and polling cleanup.
- Verification: focused Web and API tests, harness, lint, typecheck, and build passed. No AWS or Terraform mutation ran.
