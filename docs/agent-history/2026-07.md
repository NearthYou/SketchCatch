...
## Archived active Settings and Reverse Engineering records merged on 2026-07-24
...
## Archived from the active log on 2026-07-24
...
- Authenticated browser QA was not repeated; no DB schema, migration, cloud mutation, deployment, or CI/CD handoff was performed.
...
- Added a temporary Workspace AI rebuild baseline for the next UI; the completed plan was later removed during documentation consolidation.
...
- Split the legacy deployment panel into a shared console shell with independent managed deployment and CI/CD screens.
...
- Added deployment-aware cost contracts across managed deployment and CI/CD, including Destroy lifecycle handling.
...
- Completed:
  ...
  - Added a clarification boundary for multi-region API/RDS requests because Terraform Preview and managed deployment currently use one AWS provider region.
- Verification:
...
- Completed:
  - Diagnosed static portfolio, CI/CD EC2 handoff, and APAC mobile API with voice transcription prompts through ArchitectureJson, DiagramJson, and right-panel resource-list validity checks.
  ...
  - Fixed CI/CD handoff answers so natural CI/CD wording materializes CodeStar Connection, CodePipeline, CodeBuild, CodeDeploy, S3 artifacts, and the EC2 ASG runtime together.
...
- Chrome verification recreated `whiskend/audience-live-check`, saved 33 Terraform resources, passed deployment checks, and completed an AWS-backed Plan with `+33 ~0 -0 +/-0`. No Apply or cloud mutation was run.
...
- Goal: Remove unsupported inference from the `audience-live-check` draft and prove that its generated Terraform is valid before managed deployment.
- Completed:
...
- Goal: Correct the Repository-derived Fargate topology, prove managed deployment in AWS, and leave no deployment-owned infrastructure behind.
- Completed:
...
- Live verification:
  - deployment check returned zero findings.
...
- Completed:
  ...
  - Applied the generated architecture with 33 AWS resources through managed deployment.
...
- Verification:
  ...
  - managed deployment Plan/Apply (`33 to add`) and approved Destroy (`33 to destroy`).
...
- Confirmed the 33-resource `audience-live-check` managed deployment remained `DESTROYED`.
...
- Dev brought in ECS GitOps persistence and cleanup evidence, production ECS deployment speed optimization, live sandbox Direct recovery hardening, deployment sandbox E2E gates, Web UI clarity/accessibility improvements, dashboard navigation/copy simplification, and reference template branch integration records.
...
- Production request `req-c2` was a local CI/CD precondition conflict, not a duplicate GitHub resource: the target repository had no SketchCatch branch or PR, and project `0bdf56aa-68b7-4382-b37f-31d8996136c1` had no `project_deployment_targets` row.
...
- `origin/dev` was fetched and merged into this branch on 2026-07-15; incoming dev state includes the fail-closed three-stage sandbox orchestration contract, standalone AWS SAM and CodeDeploy application units, application-local static install roots, generated artifact cleanup, Web clarity/accessibility, dashboard copy, ECS deployment speed, and reference template updates.
...
#### 2026-07-15 - Diagnose and fix misleading CI/CD handoff conflict
...
- Preserved dev's deployment/GitOps persistence, production ECS speed, Web clarity/accessibility, dashboard copy, reference templates, notification, and infrastructure updates.
...
- Restored the CI/CD handoff entry point in `CicdConsoleScreen` against the current backend contract rather than copying the reverted legacy panel.
...
- No real credentials, live AWS mutation, Terraform apply/destroy, user artifact upload, or user CI/CD handoff was performed.
...
#### 2026-07-17 - Repair managed deployment result layout
...
- Focused managed deployment tests pass 14/14; harness check passes; browser verification confirmed the repaired history and action layout. No Terraform or AWS mutation was performed.
...
- Focused typography and Diagram checks pass 126/126; browser QA found only computed 400 and 700 weights with no unexpected values. Harness, lint, typecheck, production build, and diff checks pass. The unnecessary broad test run stopped on one unrelated existing CI/CD readiness contract failure in `packages/types`; no further broad test was run. No dependency, lockfile, database, deployment, cloud, or Git handoff change was made.
...
- CI now runs every API deployment and CI/CD test instead of a selected subset. The CI-equivalent matrix passes 446 API and 65 Web checks; lint, typecheck, and build pass with one unrelated existing API test-file unused-import warning.
...
- GitHub Actions now includes the managed deployment route suite and all four CI/CD route suites. The CI-equivalent matrix passes 523 API and 66 Web checks, the deployment route suite passes 64/64, and root typecheck passes.
...
### 2026-07-18 - Repair CI/CD readiness after successful managed deployment
...
- Workspace lint passed with one unrelated existing managed deployment warning; harness and browser checks confirmed the Agent Review presentation. The current board had no Terraform error, so the Error Analysis result was not visually reproduced.
...
- Focused Workbench tests pass 31/31; web lint passes with one unrelated managed deployment warning. Chrome confirmed medium cards render yellow on the current localhost review result.

#### 2026-07-17 - Normalize managed deployment action buttons

- Fixed managed deployment action buttons at 152x44 so two actions plus their gap stay within the 320px result rail.
...
- Focused managed deployment tests pass 15/15, and browser-computed layout confirms a 312px two-button footprint inside the 320px rail. No Terraform or AWS mutation was performed.
...
- Focused presentation and Workbench tests pass 26/26; web lint passes with one unrelated managed deployment warning. Chrome DOM verification confirmed the two groups and separated problem/correction lines.
...
- No DB schema, migration, cloud mutation, Terraform execution, deployment, or CI/CD handoff was performed.
...
- Kept managed deployment actions sequenced as Destroy Plan -> approval -> Destroy, without showing a second Destroy Plan action while approval is pending.
...
- No real cloud mutation, Terraform apply/destroy, project deletion, DB migration, or CI/CD handoff was performed.
...
- Added the approved executive managed deployment layout and real-data Deployment History KPIs, filters, and master-detail view without changing deployment gates or handlers.
...
- Merged `origin/dev` at `fcee2825`, preserved both workstreams, removed duplicated GitHub App environment entries, and repaired two semantic auto-merge regressions in the CI/CD health check and GitHub authorization fixture.
...
- Provider-schema validation passes all 155 enabled managed resources and all 5 enabled data sources. managed deployment safety remains unchanged: 61 resource types are plan-allowed and 94 stay blocked by the existing allowlist.
...
- managed deployment now checks the current project deployment target before saving or preparing a Full Stack/Application run and links missing setup to the existing CI/CD Delivery surface.
...
- GitHub OAuth remains a SketchCatch login mechanism only; CI/CD Repository settings now use the connected GitHub App installation token with no user OAuth fallback endpoints or UI.
...
- Fixed managed deployment so a successful Plan approval selects the deployment step instead of leaving the user on the disabled approval panel.
...
- The preference is stored under a legacy local-storage key and has focused unit coverage.
...
### 2026-07-19 - Harden managed deployment Apply recovery boundaries
...
### 2026-07-19 - Document managed deployment Apply audit improvements
...
### 2026-07-19 - Restore detailed managed deployment preparation errors
...
- Added a caller-level regression that failed against the generic fallback and passed after reconnecting the formatter; focused managed deployment checks pass 31/31.
...
- Confirmed the warning action only switches to the CI/CD screen and that the prior managed deployment warning remains visible after a screen round trip because it is not invalidated by Delivery profile reloads. The server-side gate remains fail-closed and target saving does not deploy or mutate AWS.
...
### 2026-07-19 - Synchronize saved Delivery targets with managed deployment

- Propagated successful deployment-target saves through the Delivery console and cleared stale managed deployment prerequisite presentation while retaining the fresh target and AWS connection checks before Plan preparation.
...
### 2026-07-19 - Restore managed deployment validation recovery
...
- Fixed managed deployment step selection so a previously selected idle deployment step falls back to the current validation step. The failed Workspace now exposes both `저장 후 검증 실행` and `Repository 빌드 권한 다시 확인` instead of an empty action area.
...
### managed deployment and CI/CD safety
...
- Moved Plan-time checkout verification and secret-safe Static Site/API output evidence into Phase 3. The Plan CTA only opens managed deployment and never starts Plan or Apply itself.
...
- No database migration, dependency change, cloud mutation, deployment action, or CI/CD handoff was performed.
...
- No dependency, lockfile, database migration, Terraform execution, cloud mutation, Deployment action, or CI/CD handoff was performed.
...
- No dependency, lockfile, database migration, Terraform execution, cloud mutation, deployment, or CI/CD handoff was performed.
...
- AWS read-only evidence could not be refreshed because this session has no configured credentials. No dependency, lockfile, migration, cloud mutation, production deployment, push, or CI/CD handoff was performed.
...
## Archived active records merged from latest dev (harness-diet 2026-07-24)
