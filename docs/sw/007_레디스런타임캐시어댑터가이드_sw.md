...
flowchart TD
  ...
  G -->|성공| H["Redis + local fallback dual path"]
...
- Redis cache miss나 장애가 있어도 최종 기록은 RDS/S3에서 확인한다.
...
6. `get`은 Redis hit면 JSON parse 결과를 반환하고, miss/failure면 fallback을 조회한다.
