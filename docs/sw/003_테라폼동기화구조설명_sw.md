...
## #27 DiagramJson 기반 Terraform 순수 변환기

#27의 목표는 `DiagramJson` 객체를 받아 Terraform 코드 문자열을 만드는 것이다.

핵심 함수는 아래 service에 있다.
...
입력은 보드 상태인 `DiagramJson`이고, 출력은 Terraform HCL 문자열이다.

```mermaid
flowchart LR
  DiagramJson["DiagramJson.nodes"] --> Filter["resource node만 선택"]
  Filter --> Params["node.parameters 확인"]
  Params --> Render["Terraform block 렌더링"]
...
POST /terraform/validate/prepare
```

`/terraform/generate`는 `DiagramJson`을 받아 #27의 `generateTerraformFromDiagramJson`을 호출한다.

`/terraform/validate`는 `mode: "static"`이면 정적 diagnostics만 반환하고, `mode: "full"`이면 정적 diagnostics를 먼저 통과한 뒤 API 서버의 임시 디렉터리에서 `terraform init -backend=false`와 `terraform validate -json`만 실행한다. 여러 파일이 있으면 `terraformFiles`를 함께 보내 같은 Terraform module로 검증한다. 다만 editor 검증 경로는 외부 module/backend/provider 설정을 실행하는 곳이 아니므로 사용자 HCL의 `module`, `provider`, `terraform` root block은 CLI 실행 전에 막는다.

`/terraform/validate/prepare`는 Terraform panel 진입 시 provider plugin cache를 미리 준비하기 위한 endpoint다. 실패해도 저장 action을 자동 차단하지 않고, 저장/manual Validate 때 full validation이 다시 실행된다.
...
`mode: "static"` diagnostics는 Terraform을 실제로 실행하지 않는다.
...
현재 editor validation은 이 원칙을 유지하면서 `full` mode에서만 backend/API가 격리된 temp workspace로 CLI 검증을 수행한다. 프론트엔드는 여전히 Terraform CLI를 직접 실행하지 않는다.

`terraform plan`, `apply`, `destroy`는 Deployment 흐름에서 별도 승인과 로그, secret masking을 갖춘 뒤 처리한다. Editor validation의 `terraform validate`는 실제 cloud mutation이 아니며 Deployment stage와 섞지 않는다.
