...
## #28 Terraform editor와 정적 diagnostics

#28의 목표는 #27에서 만든 Terraform을 웹에서 확인하고, 실제 Terraform CLI 없이 기본적인 문제를 알려주는 것이다.
...
`/terraform/validate`는 Terraform 문자열을 받아 정적 diagnostics를 반환한다.
...
여기서 Terraform CLI를 바로 실행하지 않는 이유는 안전성과 책임 분리 때문이다.
...
그래서 #28은 정적 diagnostics만 제공한다.

실제 `terraform validate`, `plan`, `apply`는 Deployment 흐름에서 별도 승인과 로그, secret masking을 갖춘 뒤 처리한다.
...
이 선택은 초보자에게는 “코드 값만 바꾸고 그림의 구조는 건드리지 않는다”로 이해하면 된다.

전문가 관점에서는 mutation scope를 `parameters.values`로 제한해서 graph topology와 resource identity를 보호하는 선택이다.
...
전문가 관점에서 이 parser는 일반 HCL parser가 아니라 “생성기 output round-trip을 위한 bounded parser”다.
