...
`apps/api/src/services/terraform/diagram-to-terraform.ts` 파일을 만들고 아래 코드를 그대로 작성한다. helper를 나중에 상상해서 추가하는 방식이 아니라, 처음부터 전체 helper를 포함해서 만든다.
...
`apps/api/src/services/terraform/diagram-to-terraform.test.ts` 파일을 만들고 아래 코드를 그대로 작성한다.
...
> 현재 구현 기준: 이 섹션은 초기 정적 diagnostics 작업의 기록이다. 최신 `TerraformValidateRequest`/`TerraformValidateResponse` 계약은 `docs/data-models.md`의 `Terraform 생성과 Editor 검증 DTO`를 따른다. 현재 `/api/terraform/validate`는 `mode: "static" | "full"`을 지원하고, `full` mode에서는 API 서버에서 `terraform init -backend=false`와 `terraform validate -json`만 실행한다.
