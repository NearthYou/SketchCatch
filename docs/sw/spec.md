...
- production은 API와 web ECS service를 각각 autoscaling `min=1`, `max=2`로 유지해 비용을 우선하고 부하가 있을 때만 두 번째 task를 실행한다.
...
Phase 1에서는 nginx, web, api를 한 task에 묶어 첫 전환 리스크를 줄였다. Phase 8 steady state에서는 nginx를 제거하고 ALB가 API와 web service로 직접 path routing한다. cutover 검증 후 warm EC2/SSM/nginx와 legacy ECS service를 제거하고, 암호화된 AMI와 검증 artifact를 사용하는 cold rollback만 유지한다.
...
- cutover 기간에는 EC2와 ECS를 병렬 운영했고, 완료 후 비용 절감을 위해 EC2/legacy ECS warm rollback을 제거한다.
...
- Route53/ACM은 `edge`, S3/RDS/Redis는 `data`, opt-in cold rollback은 `legacy-rollback` state로 격리한다.
...
- production API와 web service는 각각 Application Auto Scaling `min=1`, `max=2`로 실행한다. 평소에는 service별 1 task만 유지하고 CPU 부하가 있을 때만 2 task까지 확장한다.
...
- EC2 rollback, 기존 ALB와 legacy ECS service는 cutover 검증 후 제거하고 encrypted AMI, 마지막 검증 image artifact, Terraform/runbook만 보관한다.
