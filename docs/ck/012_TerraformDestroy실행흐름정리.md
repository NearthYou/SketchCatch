...
현재 destroy backend API는 준비되어 있지만, `apps/web`에는 destroy cleanup 버튼과 API helper가 아직 실제 연결되어 있지 않다.

현재 web 쪽에서 확인되는 destroy 관련 UI는 리소스 단위 destroy 버튼 placeholder 정도다.

따라서 사용자 화면 연결은 후속 작업에서 해야 한다.

후속 UI 연결 시 필요한 최소 흐름은 아래와 같다.
...
프론트엔드에서 직접 Terraform이나 AWS SDK를 실행하면 안 된다.

반드시 backend API를 호출해야 한다.
