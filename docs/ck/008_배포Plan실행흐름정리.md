...
- `tfplan` 바이너리 S3 저장
- Plan 생성 시점의 Terraform artifact hash와 Plan artifact metadata RDS 저장
...
- Plan 이후 Terraform artifact가 바뀌었거나 Plan artifact에 원본 hash가 없으면 승인 차단
...
6. 현재 S3 Terraform artifact hash가 Plan 생성 시점의 hash와 같은지 확인한다.
7. 승인 시점의 `terraformArtifactId`, Terraform artifact hash, `tfplan` hash, AWS account/region을 Deployment에 저장한다.
8. Deployment의 blocked 상태를 해제한다.
...
8. 복원된 Terraform artifact sha256 계산
9. Deployment status = RUNNING 저장
10. terraform init 실행과 로그 저장
11. terraform validate 실행과 로그 저장
12. terraform plan -out=tfplan 실행과 로그 저장
13. terraform show -json tfplan 실행
14. show JSON stdout은 저장하지 않고 planSummary만 파싱
15. show stderr만 plan 로그로 저장
16. risk/block 상태 계산
17. tfplan 바이너리 S3 업로드와 sha256 계산
18. deployment_plan_artifacts metadata 저장
19. deployments.currentPlanArtifactId 갱신
20. Deployment status = PENDING 저장
21. 임시 workdir cleanup
```
...
`terraformArtifactSha256`은 Plan 생성 시점의 Terraform artifact 내용 hash다. 기존 row 마이그레이션을 위해 nullable이지만, 값이 없는 Plan artifact는 승인할 수 없고 새 Plan을 다시 생성해야 한다.
...
- Plan artifact에는 `tfplan` hash와 Plan 생성 시점의 Terraform artifact hash를 함께 저장한다.
- raw `terraform show -json` 결과는 저장하지 않는다.
- destructive change는 Plan 저장 후 `risk_analysis`로 막는다.
- 새 Plan 또는 init이 시작되면 이전 approval snapshot을 비워 오래된 승인으로 Apply할 수 없게 한다.
- Approval DB update는 현재 Plan pointer, `PENDING` 상태, `missing_approval` block 조건이 맞을 때만 성공한다.
- Approval 시점에 현재 S3 Terraform artifact hash를 다시 계산해 Plan 생성 시점 hash와 다르면 승인하지 않는다.
- Apply precondition helper는 DB metadata hash가 아니라 Apply 직전에 다시 계산한 현재 `tfplan` hash를 받아 비교한다.

이번 보안 보강에서 고친 것:

1. Plan 생성 시 복원된 Terraform artifact hash를 `deployment_plan_artifacts.terraform_artifact_sha256`에 저장한다.
2. 기존 row 호환을 위해 DB 컬럼은 nullable로 추가하되, hash가 없는 Plan artifact는 승인하지 않고 새 Plan 생성을 요구한다.
3. 승인 직전에 S3 Terraform artifact를 다시 내려받아 Plan 생성 시점 hash와 비교한다.
4. 새 init 또는 새 Plan 저장 시 기존 approval snapshot을 초기화한다.
5. 승인 저장은 현재 Plan pointer와 blocked 상태가 그대로일 때만 성공하도록 조건부 update로 처리한다.
6. Apply 검증 helper가 현재 S3 `tfplan` hash를 외부에서 재계산해 넘기도록 계약을 바꿨다.
...
- Terraform artifact hash 저장과 승인 전 drift 차단
- hash가 없는 legacy Plan artifact 승인 차단
...
2. Apply 직전 S3의 Terraform artifact hash와 `tfplan` hash를 다시 계산한다.
3. 승인한 Terraform artifact hash 또는 `tfplan` hash와 현재 S3 hash가 다르면 apply를 막는다.
