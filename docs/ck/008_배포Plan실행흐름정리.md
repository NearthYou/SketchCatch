...
범위는 사용자가 Workspace에서 `Terraform Plan 실행`을 누른 뒤, API가 Terraform artifact를 복원하고 `terraform init`, `terraform validate`, `terraform plan -out=tfplan`, `terraform show -json tfplan`을 실행해 Plan 결과를 저장하고 화면에 돌려주는 단계까지다.

아직 실제 AWS 리소스를 변경하는 `approve`와 `apply`는 이 문서의 구현 완료 범위에 포함하지 않는다.
...
- Plan 실행 로그 저장과 조회
- Workspace 배포 패널에서 Plan 실행, summary, blocked reason, 로그 표시

아직 남은 범위:

- Plan 승인 API
- blocked Deployment 승인 차단
- 승인한 `tfplan`과 apply 대상 `tfplan` 일치 검증
...
현재는 approve/apply API가 아직 없으므로, 실제 승인 차단 로직은 다음 단계에서 구현한다.
...
다음 구현은 approval과 apply다. 이때 반드시 이어서 확인해야 하는 것:

1. 사용자가 승인한 `currentPlanArtifactId`를 승인 기록으로 고정한다.
2. 승인 시점의 `terraformArtifactId`, `tfplan sha256`, AWS account, region을 저장한다.
3. Apply 직전 S3의 `tfplan` hash를 다시 계산한다.
4. 승인한 hash와 현재 S3 hash가 다르면 apply를 막는다.
5. 승인한 AWS account/region과 현재 AssumeRole 결과가 다르면 apply를 막는다.
6. blocked Plan은 승인할 수 없게 한다.
7. `terraform apply tfplan`만 실행한다.
8. Apply stdout/stderr도 마스킹해 저장한다.
9. 성공 후 생성 리소스와 `terraform output -json`을 저장한다.
10. cleanup/destroy 흐름 없이는 위험 리소스 apply 범위를 넓히지 않는다.
