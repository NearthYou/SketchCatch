...
이 변경은 사용자가 이미 분석한 GitHub Repository를 CI/CD에 연결할 때 같은 Repository를 다시 선택하고 다시 분석하는 중복 단계를 제거한다. 사용자는 Repository 분석 결과에서 GitHub App 권한을 연결한 뒤 `프로젝트 배포 타깃`과 `GitOps 감시 설정`을 저장하고, 기존 분석 결과로 돌아가 선택한 `ECS Fargate container app` Board를 생성한다.
...
3. 사용자가 `ECS Fargate container app`을 선택한다.
...
13. 사용자는 재분석 없이 `ECS Fargate container app` Board 생성 흐름을 계속한다.
...
기본값은 `ecs-fargate-container-app` 선택과 연결된 Repository 분석 evidence가 있을 때만 적용한다. 기존에 저장된 `ProjectDeploymentTarget`이 있으면 저장값을 우선하고 자동 기본값으로 덮어쓰지 않는다.
...
- ECS 기본값 helper가 project slug, Dockerfile root/path, commit SHA와 resource names를 결정적으로 만든다.
- 기존 저장 target은 자동 기본값으로 덮어쓰지 않는다.
...
- 기본값이 선택한 ECS Fargate Board의 AWS resource values와 일치한다.
