...
## Golden-path Repository owner 동등성

`whiskend/audience-live-check`와 `jh-9999/audience-live-check`는 같은 데모 애플리케이션 계약을 가진 별도 GitHub Repository다. 사용자가 어느 URL을 입력하더라도 공개 Repository 분석은 각 Repository의 실제 branch head와 evidence를 독립적으로 읽고, 동일한 evidence에는 동일한 ECS Fargate 추천과 질문 흐름을 제공해야 한다.

이 동등성은 owner별 runtime 분기나 한 Repository 결과를 다른 Repository에 복사하는 cache alias로 구현하지 않는다. 추천 입력은 Repository owner가 아니라 Dockerfile, Application Unit, framework, 정적 frontend output, 데이터 영속성, ECS/Fargate 배포 근거다. 따라서 owner만 바뀐 fork는 기존 일반 분석 경로를 그대로 통과한다.

두 Repository URL은 다음 계약을 만족해야 한다.

- `.git` suffix와 owner 대소문자를 정규화하되 실제 owner/name으로 해당 Repository를 조회한다.
- 각 Repository의 실제 commit SHA를 `repositoryRevision`으로 유지한다.
- React/Vite frontend, Express/Node.js API, `apps/api/Dockerfile`, DB 없음, ECS/Fargate golden-path 근거가 같으면 `ecs-fargate-container-app`을 1순위로 추천한다.
- 사용자가 frontend 포함 질문에 같은 답을 선택하면 동일한 ECS Fargate Architecture Draft 생성 경로로 진행한다.
- GitHub App 연결은 분석한 owner/name과 정확히 일치하는 Repository에만 연결한다. fork라는 이유로 다른 owner의 Repository 권한이나 revision을 재사용하지 않는다.

회귀 검증은 두 owner의 `.git` URL이 정상 정규화되는지, 동일한 golden-path evidence가 AI 재정렬 이후에도 ECS Fargate를 1순위로 유지하는지 확인한다. 별도의 URL allowlist나 데모 전용 추천 API는 추가하지 않는다.
...
초기 UI 검증 기간에는 callback의 `프로젝트 배포 타깃 저장`과 `설정 저장` 버튼을 숨기고, 두 설정 아래에 단일 `확인` 버튼을 표시한다. 이 버튼은 현재 draft로 배포 타깃 저장 API를 먼저 호출하고 성공하면 GitOps 감시 설정 저장 API를 호출한다. 두 저장이 모두 성공할 때만 기존 Repository 분석 화면으로 돌아가며, 하나라도 실패하면 callback에 남아 해당 설정 오류를 보여준다. 일반 프로젝트 설정 화면의 개별 저장 버튼과 API 계약은 유지한다. 이 임시 순차 저장 UI는 이후 통합 저장 endpoint를 설계할 때 교체할 수 있다.

### 완료 액션 바

단일 확인 동작은 두 설정 카드와 분리된 완료 액션 바에 배치한다. 액션 바 왼쪽은 이 동작이 `배포 타깃`과 `GitOps 감시 설정`을 함께 저장한 뒤 Board 선택으로 이동한다는 결과를 설명하고, 오른쪽은 `설정 저장 후 계속` 기본 버튼을 제공한다. 버튼의 최소 높이는 48px이며 키보드 focus, hover, active, disabled 상태를 구분한다. 저장 중에는 spinner와 `설정 저장 중` 문구를 표시해 중복 제출을 막는다.

저장 실패 메시지는 액션 바 안에서 버튼과 가까운 위치에 표시하고 기존 callback 화면을 유지한다. 640px 이하 화면에서는 안내와 버튼을 한 열로 쌓고 버튼을 전체 너비로 확장한다. 이 변경은 저장 순서, API payload 또는 복귀 조건을 바꾸지 않는다.

아래의 두 저장 성공 조건은 단일 확인 버튼이 내부적으로 충족해야 하는 현재 계약이다.
...
- `whiskend/audience-live-check.git`과 `jh-9999/audience-live-check.git`을 각각의 owner와 동일한 Repository name으로 정규화한다.
- 두 owner의 동일한 golden-path evidence가 ECS Fargate를 1순위로 유지한다.
