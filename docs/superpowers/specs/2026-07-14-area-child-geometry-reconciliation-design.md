...
Architecture Board의 영역 노드가 다른 영역의 자식이 될 수 있음을 드래그 중에 명확히 표시하고, 자식 변경이 확정된 시점에만 크기를 다시 계산하도록 한다. 자식이 추가되면 실제 위치에 따른 부족분을 계산하지 않고 자식 크기의 `1.3` 배만큼 부모 영역을 확장한다. 자식이 줄어들면 남은 자식 수와 크기에 맞춰 같은 규칙으로 축소한다. 마지막 자식이 제거되면 자동 확장 전 사용자가 설정했던 위치와 크기로 정확히 복원한다.

## 현재 문제

- 최초 구현에서는 영역 노드를 다른 영역으로 드래그하면 부모 후보가 하이라이트되지 않았다. 당시 `findInnermostAreaDropTarget()`은 드래그 중인 노드가 영역이면 즉시 `null`을 반환했지만, drag stop의 부모 할당은 자식 영역 전체가 후보 영역에 포함되면 정상 처리해 드래그 피드백과 최종 결과가 일치하지 않았다.
- 최초 자동 확장 로직은 영역 또는 일반 리소스가 부모 영역에 들어오면 자식 크기의 `1.5` 배만큼 부모와 조상 영역을 누적 확장했다. 자식을 삭제하거나 다른 영역으로 이동해도 삭제 경로는 노드와 부모 관계만 정리해 확장된 영역 크기가 줄어들지 않았다.
- baseline 재조정 구현 이후에는 자식 bounding box와 padding을 합쳐 실제로 부족한 방향만 확장한다. 이 방식은 공간을 절약하지만, 사용자가 원하는 기존의 단순하고 넉넉한 확장감보다 변화량이 작다.
...
- 자식이 있는 동안 영역의 너비와 높이는 각각 `기준 크기 + 현재 직접 자식 크기 합계 × 1.3`으로 계산한다. 자식의 실제 위치와 경계 부족분은 계산에 사용하지 않는다.
- 확장된 영역의 중심은 기준 영역의 중심과 동일하게 유지해 좌우와 상하로 같은 양만큼 늘어난다.
- 자식이 일부 제거되거나 다른 영역으로 이동하면 남은 직접 자식의 크기 합계로 다시 계산해 축소한다.
- 마지막 직접 자식이 제거되면 저장한 기준 `position`과 `size`를 복원하고 기준 metadata를 제거한다.
- 자식이 있는 동안 사용자가 영역을 직접 리사이즈하면 완료된 수동 geometry를 새로운 기준값으로 저장한 뒤, 현재 직접 자식 크기의 `1.3` 배 합계를 다시 더한다.
...
직접 자식이 하나 이상인 영역의 목표 크기는 다음과 같다.

```text
targetWidth = baseline.width + sum(directChild.width * 1.3)
targetHeight = baseline.height + sum(directChild.height * 1.3)
targetX = baseline.centerX - targetWidth / 2
targetY = baseline.centerY - targetHeight / 2
```

계산은 자식의 위치나 부모 경계에서 부족한 픽셀을 보지 않는다. 따라서 이미 기준 영역 안에 충분히 들어오는 자식도 부모 크기를 늘린다. 직접 자식이 없으면 baseline을 그대로 복원한다. baseline도 없는 legacy 영역은 현재 geometry를 유지한다.

중첩 영역은 가장 깊은 영역부터 계산한다. 바깥 영역은 직접 자식인 안쪽 영역의 최종 너비와 높이를 `1.3` 배 계산에 사용한다. 같은 현재 노드 집합은 이벤트 이력과 무관하게 항상 같은 결과를 만들며, 자식 추가와 제거를 반복해도 누적 오차가 생기지 않는다. 순환하거나 존재하지 않는 부모 참조는 방문 집합으로 중단하고 해당 경로를 변경하지 않는다.
...
`area-node-geometry.ts`의 자식 bounding box와 padding 합집합 계산을 직접 자식 크기 합계 기반 `1.3` 배 계산으로 교체한다. baseline 저장·복원, 영향 영역 탐색, deepest-first 처리, 확정 이벤트 연결은 유지한다. 모듈은 노드 배열과 영향받은 영역 ID를 받아 변경된 영역 노드만 새 객체로 반환하며, 크기 계산 자체는 React와 React Flow에 의존하지 않는다.
...
- 첫 자식 추가 시 기존 영역 geometry가 baseline으로 저장되고 부모 너비와 높이에 각각 자식 너비와 높이의 `1.3` 배가 더해진다.
- 자식이 기준 영역 안에 이미 들어와 있어도 같은 `1.3` 배 확장을 적용한다.
- 여러 직접 자식이 있으면 각 자식 너비와 높이의 `1.3` 배 합계를 baseline에 더한다.
- 일부 자식 제거 시 남은 직접 자식 크기 합계 기준으로 축소된다.
- 마지막 자식 제거 시 원래 baseline 위치와 크기가 복원되고 metadata가 제거된다.
- 자식이 다른 영역으로 이동하면 이전 영역과 새 영역을 같은 transaction에서 재조정한다.
- 중첩 영역은 안쪽부터 바깥쪽으로 재조정되고, 바깥 영역은 안쪽 영역의 최종 크기를 사용한다.
