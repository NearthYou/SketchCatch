...
Resource `label`은 표의 대소문자로 저장한다. Architecture Board는 기존 표시 규칙에 따라 영어가
포함된 Resource 이름을 대문자로 렌더링하며, 이 작업에서 강제 대문자 표시를 제거하거나
Template 전용 분기를 추가하지 않는다. Resource 패널과 설명은 저장된 `label`의 대소문자를
유지한다.
...
- Architecture Board의 기존 Resource label 대문자 표시 방식 변경
