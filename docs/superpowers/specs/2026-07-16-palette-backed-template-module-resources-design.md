...
- 일부 Template/Module Resource가 `enabled: false`인 catalog item을 사용해 왼쪽 Palette에서 직접 사용할 수 없고, 대응 설정 schema도 비어 있다.
...
5. Curated Module materialization은 Template와 같은 Palette node factory를 호출하되, source Terraform file이 없는 `workspace-seed` node는 Palette default parameters로 전환한다. 비어 있지 않은 authored label, 상대 위치, 관계, containment와 해결된 parameter 값은 보존하고, 빈 label과 일반 Resource 타일의 크기는 Palette contract를 따른다.
6. Module provenance는 Web, save API, Terraform API에서 동일하게 허용한다.
7. 새 SketchCatch Workspace는 캡처 당시의 AWS provider 5.52 고정값 대신 저장소 공통 5.x baseline을 사용한다. 캡처 원문과 hash는 변경하지 않는다.
8. `terraform init/validate`는 HCL 구조 검증이다. 실제 AWS plan/apply나 Direct Deployment safety 승인을 의미하지 않는다.
...
- Palette에 새로 노출한 Resource는 parameter panel schema를 제공하고 AWS provider schema 기반 감사에서 필수 필드 누락이 없어야 한다.
