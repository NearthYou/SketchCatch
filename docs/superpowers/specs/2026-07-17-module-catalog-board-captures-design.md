...
## Dashboard Template 시작 흐름

### 문제

`/dashboard/templates`의 시작 Link는 선택한 `templateId`를 전달하지만 `/workspace/new`는
`mode=template`이면 항상 전체 Template Catalog를 먼저 연다. 사용자는 목록 아래에서 시작
버튼을 누른 뒤 화면 위에서 프로젝트 이름을 입력하고, 같은 Template을 다시 찾아 내려가야 한다.

### 결정

유효한 `templateId`와 함께 진입하면 선택한 Template 상세 화면을 바로 연다. Dashboard에 별도
생성 modal이나 중복 프로젝트 생성 로직은 만들지 않는다. 일반적인
`/workspace/new?mode=template` 진입이나 유효하지 않은 `templateId` 진입은 기존 Catalog를 연다.

Template 상세의 오른쪽 정보 영역에서 제목과 설명 바로 아래에 프로젝트 이름 입력란과
`이 템플릿으로 시작` 버튼을 한 묶음으로 배치한다. Resource 수와 tag 같은 보조 정보보다 먼저
노출해 첫 화면에서 입력과 실행을 마칠 수 있게 한다. 좁은 화면에서는 정보 영역을 캡처보다 먼저
배치한다.

프로젝트 이름이 비어 있으면 같은 묶음 안에서 오류를 표시하고 입력란에 focus한다. 선택한
Template ID·version, 사용자가 입력한 프로젝트 이름, 새 프로젝트의 초기 Board 저장 방식은
기존 단일 시작 흐름과 session 복원 계약을 그대로 사용한다.
...
- 사용자용 문구가 모든 Module을 포함하고 raw Terraform 타입·내부 version을 노출하지 않는지
  검증한다.
- Dashboard에서 전달한 유효한 `templateId`가 Catalog가 아닌 해당 상세 화면을 여는지 검증한다.
- 상세 화면이 프로젝트 이름 입력란과 시작 버튼을 동시에 노출하는지 검증한다.
- 빈 이름은 같은 위치에서 오류를 표시하고 입력란에 focus하며 프로젝트를 생성하지 않는지
  검증한다.
- `templateId`가 없거나 유효하지 않으면 기존 Catalog 흐름을 유지하는지 검증한다.
- 선택한 Template ID·version과 프로젝트 이름 복원, 초기 Board 적용 회귀를 검증한다.
...
- Dashboard 안에 별도 프로젝트 생성 modal 또는 두 번째 생성 로직 도입
- 일반적인 Template Catalog 검색·정렬·tag 동작 변경
