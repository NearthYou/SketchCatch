...
각 노드는 `x`, `y`, `width`, `height`, `cornerRadius`와 lane별 `entry`, `exit` 좌표를 가진다. 정적 레일은 다음 두 경로를 모두 그린다.
...
노드 사이에서도 두 경로를 합치지 않는다. `upper` exit은 다음 노드의 `upper` entry로, `lower` exit은 다음 노드의 `lower` entry로 각각 연결해 시작부터 끝까지 두 개의 평행 connector를 유지한다. 두 connector는 서로 다른 좌표를 사용하며 중앙 shared connector를 만들지 않는다. 레일은 노드 border 바깥 `8~12px` 간격을 유지하며 노드 내부를 지나지 않는다.

ASG 뒤에서는 upper/lower 레일이 각각 실제 표시 EC2 슬롯 수만큼 분기한다. EC2 하나당 두 개의 분기 곡선이 존재하며 두 곡선은 target 노드의 동일 lane entry로 이어진다.

- EC2 0개: ASG까지만 표시하고 뒤쪽 분기를 그리지 않는다.
- EC2 1개: 중앙에 배치한 EC2 한 개로 upper/lower 두 선을 연결한다.
- EC2 2개: 위 EC2와 아래 EC2 각각에 upper/lower 두 곡선을 연결한다.
...
- 모바일의 left/right connector와 EC2 branch도 서로 합쳐지지 않고 끝까지 별도 좌표를 유지한다.
...
1. geometry 단위 테스트
   ...
   - upper/lower connector가 서로 다른 좌표로 각 lane의 exit과 다음 lane entry를 연결한다.
   - desktop connector와 EC2 branch에 `shared` lane이 존재하지 않는다.
   - EC2 슬롯이 0/1/2일 때 분기 수가 각각 0/2/4다.
   - 모바일 left/right connector와 branch도 동일한 이중 레일 계약을 지킨다.
...
- Audience 시작부터 EC2 도착까지 upper/lower 또는 left/right 레일이 한 좌표로 합쳐지는 구간이 없다.
