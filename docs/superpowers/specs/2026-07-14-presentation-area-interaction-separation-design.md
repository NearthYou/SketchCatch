...
`aws_autoscaling_group`은 모든 보드에서 실제 Area 리소스로 복원한다. 템플릿에 배치되는 ASG에는 `presentationArea: true`도 명시해 작성된 배경 표현임을 보존하지만, ASG는 기본 Area 타입이므로 presentation 전용 프레임이 아니라 실제 상호작용 Area로 취급한다.
...
최근 템플릿 정렬 변경에서 ASG가 Area 타입에서 제외되고 일반 리소스 geometry로 정규화되었다. 이 설계에서는 ASG를 다시 기본 Area 타입에 포함하고, 기존의 ASG Area 크기를 일반 타일 크기로 축소하는 정규화도 제거한다.
...
`presentationArea`는 큰 프레임 렌더링과 기존 템플릿 레이아웃에만 남긴다. 빈 공간 상호작용과 신규 drop 부모 판정에서는 제외한다. 단, ASG처럼 원래부터 기본 Area 타입인 리소스는 `presentationArea`가 함께 있어도 실제 Area 동작을 유지한다.
...
기존 `isAreaNode()`의 시각 판정 의미를 유지하되 ASG를 기본 Area 타입에 다시 포함한다. Region, VPC, Subnet, ASG, Security Group 시각 범위와 `presentationArea` 리소스를 큰 프레임으로 렌더링한다.

### 레이아웃 컨테이너

기존 템플릿이 이미 가진 `parentAreaNodeId`를 보존하고, 컨테이너 이동 시 자식 배치가 함께 유지되게 한다. ASG는 실제 레이아웃 컨테이너로 복원하고 기존 ASG Area 크기를 유지한다. 그 밖의 presentation 전용 프레임에 대한 기존 레이아웃 geometry 계약은 바꾸지 않는다.

### 빈 공간 상호작용 Area

Region, Availability Zone, VPC, Subnet, ASG, 일반 디자인 Area만 빈 공간 클릭과 드래그를 시작할 수 있다. `presentationArea` 전용 프레임과 Security Group 시각 범위는 제외한다.

presentation 전용 여부는 플래그만으로 결정하지 않는다. `presentationArea: true`이면서 Region, Availability Zone, VPC, Subnet, ASG 같은 기본 Area 타입이 아닌 리소스만 presentation 전용으로 판정한다. 따라서 템플릿 ASG는 `presentationArea: true`가 있어도 실제 Area로 남는다.
...
`presentationArea` 전용 프레임과 Security Group 시각 범위는 새로 드래그한 리소스의 부모 후보에서 제외한다. VPC, Subnet, ASG 등 실제 Board containment Area만 후보가 된다.
...
템플릿 ASG 생성 흐름은 `presentationArea: true`를 `DiagramNode.metadata`에 기록한다. ASG는 기본 Area 타입이므로 이 플래그는 템플릿 작성 의도를 나타내며, 상호작용을 비활성화하는 presentation 전용 판정에는 사용되지 않는다.
...
7. 일반 ASG와 `presentationArea: true`인 템플릿 ASG는 모두 실제 Area로 판정된다.
8. ASG 빈 공간 선택, 자식 동반 이동, 신규 리소스 drop 부모 동작이 유지된다.
9. 템플릿 ASG는 `metadata.presentationArea: true`와 작성된 Area 크기를 가진다.
10. ASG Area가 일반 리소스 타일 크기로 정규화되지 않는다.
