...
| managed deployment    | SketchCatch가 사용자가 승인한 IaC Preview를 직접 실행                     | Terraform Plan/Apply/Destroy, approval, logs, cleanup               |
| CI/CD delivery | SketchCatch가 IaC Preview를 Source Repository PR과 외부 pipeline으로 넘김 | Terraform commit/PR, pipeline template/status, team review          |
...
managed deployment의 실제 live apply 리소스는 안정성을 위해 아래로 제한합니다.
...
Repository가 production 시작 시 필수 runtime Secret을 검사하더라도 사전 검사에 실제 Secret은 필요하지 않습니다.
Repository Analysis가 확인한 `requiredRuntimeSecrets` 이름마다 CodeBuild가 격리된 placeholder를 `docker run --env`로
주입하며, Secrets Manager 조회 권한이나 실제 배포 값을 사용하지 않습니다. 이 placeholder는 image나 runtime Task
Definition으로 전달되지 않습니다.

승인된 ECS/Fargate Terraform Apply에서는 `CHECK_IN_SIGNING_SECRET`을 48자 값으로 생성하고 Secrets Manager
Secret Version에 저장합니다. ECS execution role에는 해당 Secret ARN의 `secretsmanager:GetSecretValue`만 허용하며,
Task Definition은 `secrets` mapping으로 모든 Task에 같은 값을 주입합니다. 이후 application release는 승인된
Task Definition을 복제하고 image만 교체하므로 Secret ARN mapping을 유지합니다. `INSTANCE_ID`는 고정 주입하지 않고
각 Task hostname을 사용해 scale-out 시 `servedBy`가 Task별로 달라지게 합니다.

Repository Analysis는 `requiredRuntimeSecrets` 이름만 build contract에 기록하며 Fixed Template Board 재생성을 배포
전제조건으로 사용하지 않습니다. `full_stack` 배포 준비 단계는 저장된 Project Draft에서 준비한 `.tf` 원문을 기준으로
Secret, Secret Version, 최소 읽기 IAM policy, Task mapping, ECS Service 참조가 하나의 체인인지 검증합니다. 하나라도
빠지거나 서로 다른 리소스를 가리키면 Plan/승인 단계로 넘어가지 않고 현재 Terraform 코드의 수정·저장을 요구합니다.
Terraform artifact가 아직 없는 AI Fixed Template 생성 단계에서만 Diagram 계약을 보조 검증으로 사용합니다.
...
CI/CD application workflow는 사용자 계정 안에서 다음 read-only preflight를 실행한다.
...
- 실제 Terraform 실행은 로컬 분리 worker 또는 운영 ECS worker에서만 수행합니다.
...
- 로컬 개발은 `local_process` mode로 Terraform worker를 API hot reload와 분리합니다. 운영 ECS worker mode와 마찬가지로 API 재시작 시 active worker를 보호하며, 명시적인 legacy `in_process` mode만 실행 주체를 잃은 `RUNNING` Deployment를 `FAILED`로 정리합니다.
- Representative Use Journey나 리허설 후 생성 리소스 cleanup을 반드시 확인합니다.

### managed deployment 최적화 정책
...
CI/CD delivery는 운영 배포와 팀 리뷰를 위한 경로입니다. SketchCatch는 Terraform 파일을 Source Repository에 commit하거나 PR로 넘기고, 외부 pipeline 상태를 추적합니다.

- CI/CD handoff도 User-Accepted Change 이후에만 생성합니다.
- Source Repository token, deploy key, CI secret 원문은 응답, 로그, DB에 저장하지 않습니다.
- PR에는 IaC Preview, Plan 요약, deployment check 결과, Cost Analysis 요약을 연결합니다.
- 운영 apply는 외부 pipeline의 승인 job이나 조직 정책을 따를 수 있지만, SketchCatch는 승인 없는 apply를 권장하거나 자동 실행하지 않습니다.
- 최초 managed deployment와 설치 PR은 Terraform·Workflow·Repository/AWS 설정을 준비합니다. 이후 App은 target branch push로, Infra는 사용자가 직접 실행한 `gh workflow run sketchcatch-infra.yml --ref main`으로 따로 배포합니다.
...
- managed deployment와 CI/CD delivery는 서로 경쟁하는 선택지가 아니라 사용 맥락이 다른 실행 경로입니다.
- CI/CD 상태 polling과 long-running workflow status는 Redis Runtime Cache를 사용할 수 있지만, 최종 기록은 RDS/S3에 남깁니다.
...
managed deployment와 Pipeline Run의 성공·실패·취소는 PostgreSQL terminal trigger가 같은 transaction에서
...
`api_base_url`과 준비 시점의 ECR/cluster/service/container/port 좌표가 일치할 때만 project target의 Output URL을 원자적으로 연결한 뒤
runtime release를 시작한다. 승인된 Terraform state inventory가 증명하는 Task Definition, IAM role, ALB/Target Group, S3, CloudFront와 파생 URL 변경은 정상적인 Terraform 관리 전이로 저장하고 trusted worker가 실제 AWS 상태를 다시 검증한다. output 누락·안전성 위반·준비 좌표 불일치와 runtime release 검증 실패는
...
`infrastructure` ECS/Fargate Apply는 Terraform Output과 resource inventory를 RDS에 저장한 직후 같은 증거로
`ProjectDeploymentTarget` metadata 동기화를 시도한다. `full_stack`은 별도 readiness 동기화를 먼저 실행하지 않고,
Application Output reconciliation이 잠근 target과 준비된 `ApplicationRelease`를 한 트랜잭션에서 함께 갱신한다.
준비된 release는 좌표 fingerprint와 당시 Output URL(`null` 포함)을 함께 보존한다. 따라서 재배포 중 이전
writer나 일시적 저장 실패로 target만 새 Terraform Output을 가진 상태가 되어도, 같은 AWS account/region과
stable ECS target임을 확인한 뒤 pending release를 원자적으로 복구할 수 있다. 과거 pending release에 URL
snapshot이 없으면 다음 검증 단계에서 build artifact를 다시 만들지 않고 snapshot metadata만 보강한다.
이 후처리는 RDS와 승인된 Plan artifact만 사용하며 AWS Resource를 생성·수정·삭제하지 않는다. 필수 Output이나
Repository 빌드 근거가 불완전하면 readiness가 해당 항목을 수동 action으로 표시한다. `infrastructure`의 일시적인
metadata 동기화 오류는 Deployment log에 warning으로 남기되 이미 성공한 Terraform Apply를 `FAILED`로 되돌리지 않는다.

trusted release의 rollback baseline은 현재 `deploymentTargetFingerprint`와 같은 성공 release에서만 선택한다.
destroy/recreate 또는 account/region, ECS service, health Output URL 변경으로 target fingerprint가 달라지면
Direct와 GitOps 모두 이전 인프라의 Task Definition을 baseline으로 재사용하지 않는다. 재시작 복구와 수동
rollback도 저장된 baseline ID와 target fingerprint를 다시 대조한다.
...
8. Plan summary와 deployment check 표시
9. 사용자 승인
...
- deployment check와 Safety Gate warning은 Plan 결과에 보존하되 Plan record 자체나 Plan 승인을 blocked로 만들지 않습니다. High warning이 있어도 사용자는 Plan을 승인할 수 있으며, finding은 승인 전 검토 정보로 계속 표시합니다.
...
Phase 9부터 SketchCatch 자체 production infrastructure를 Terraform-managed IaC로 전환할 구조를 `infra/aws/production`에 둡니다. 이 경로는 사용자의 managed deployment 및 CI/CD delivery와 state, credential, workflow를 공유하지 않습니다.
...
재획득한 claim은 `updatedAt` fencing token으로 이전 worker의 늦은 성공·실패를 거부하며, 부분 정리 실패는
일반 CodeConnection 상태 새로고침이 아니라 관리 리소스 정리 재시도로만 복구합니다.

CodeConnections 생성 API는 현재 사용자에게 활성 GitHub App installation이 정확히 하나 있을 때만 AWS 요청을
시작합니다. `AVAILABLE`은 AWS 계정의 GitHub provider 승인이 끝났다는 뜻이며 project Repository 검증 완료로
사용하지 않습니다. `POST /projects/:projectId/build-environment/verify-repository-access`는 confirmed commit SHA로
build-only CodeBuild project를 실행하고 `BatchGetBuilds`의 `resolvedSourceVersion`을 비교합니다. 요청 SHA와 실제
checkout SHA가 같은 성공 build만 검증 완료로 저장하며, build ARN과 실패 사유는 secret 없이 RDS에 남깁니다.
...
Redis Runtime Cache는 Deployment, Reverse Engineering, CI/CD Integration 상태 추적을 우선 지원합니다. AI 결과물 캐싱은 2순위이며, 캐시된 결과가 RDS/S3의 원천 기록이나 Deployment Safety Gate를 대체하지 않습니다.
...
CI/CD delivery의 운영 repository 연결은 GitHub App 설치 흐름을 기본 경로로 사용합니다. Web은 GitHub App state를 직접 만들지 않고 API가 발급한 install URL로 redirect만 수행합니다.
...
`GIT_APP_PRIVATE_KEY_BASE64`는 GitHub App private key PEM을 base64로 인코딩한 값입니다. `GIT_APP_CLIENT_ID`와 `GIT_APP_CLIENT_SECRET`은 설치 소유권을 확인하고 installation을 현재 SketchCatch 사용자에게 귀속시키는 GitHub App user authorization에 사용합니다. `GIT_APP_STATE_SECRET`은 선택값이며, 비워 두면 `AUTH_TOKEN_SECRET`으로 short-lived state를 서명합니다.
...
- CI/CD pipeline status cache가 Runtime Cache를 사용할 수 있는지
...
API는 `DEPLOYMENT_WORKER_MODE=local_process` 또는 `ecs`가 설정된 경우 Terraform 실행을 API process 안에서 바로 시작하지 않고,
`deployment_jobs` row를 만든 뒤 분리 worker로 넘깁니다. 로컬 개발 예시는 detached local process를 사용해 API hot reload 후에도
Terraform 실행을 유지하고, production은 ECS `RunTask` one-off worker task를 사용합니다. 환경 변수를 생략하면 legacy
`in_process` fallback을 사용합니다.
...
| `local_process` PID가 active                   | API hot reload와 관계없이 `deployment_jobs`의 deployment를 보호하고 local worker가 계속 완료하도록 둡니다.       |
| `local_process` PID가 종료됨                   | active job을 `FAILED`로 종료하고 실제 AWS/state 상태를 다시 확인할 수 있게 deployment를 정리합니다.              |
