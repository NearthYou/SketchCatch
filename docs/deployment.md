...
Phase 8의 ECS 정상 경로는 nginx 없이 ALB가 API와 web 서비스로 직접 전달합니다. Route53 alias 생성은 기본값으로 꺼져 있으며, live ALB/Route53 변경은 별도 plan 검토와 명시적 승인이 필요합니다.
...
이 정의는 비용이 발생할 수 있는 ALB, API task, web task, CloudWatch Logs, ECR repository를 포함합니다. `ecs_desired_count = 1`은 API와 web 각각에 적용되므로 기존 단일 task 구조보다 최소 web task 1개가 추가됩니다. 실제 `terraform plan/apply`는 비용, rollback, cleanup 기준과 GitHub/AWS 변수 준비가 끝난 뒤 별도 승인으로만 실행합니다.

`docker/nginx.Dockerfile`, `docker/nginx.conf`, EC2 배포 workflow와 nginx ECR/log group은 EC2/SSM rollback 보존 기간 동안 legacy 자산으로 유지합니다. ECS steady state는 nginx image를 build/push/deploy하지 않습니다. EC2 환경을 제거하거나 nginx ECR/log group을 정리하는 작업은 rollback 보존 종료가 승인된 별도 cleanup issue에서만 수행합니다.
...
-> ECR push
-> 현재 API/web task definition 조회
-> API task definition revision 등록 및 service 안정화 대기
-> web task definition revision 등록 및 service 안정화 대기
```

두 ECS service update는 원자적이지 않습니다. API update가 성공하고 web update가 실패하면 API는 새 revision, web은 이전 revision으로 남으므로 image SHA와 각 service revision을 기록하고 실패한 service를 복구하거나 API를 이전 revision으로 되돌려야 합니다. 정상 ECS 배포에서는 DB migration을 자동 실행하지 않으며 별도 수동 workflow에서 다룹니다.

기존 단일 app service에서 분리 service로 바꾸는 최초 Terraform apply는 listener가 새 target group을 가리킨 뒤 task가 healthy해지는 구간을 만들 수 있습니다. Route53이 이미 ECS ALB를 가리킨다면 먼저 EC2 rollback 경로로 traffic을 되돌리거나 승인된 maintenance window를 확보하고, API/web target health 확인 후 ECS alias를 복구합니다. 이 migration apply와 Route53 변경은 각각 별도 plan 및 명시 승인이 필요합니다.
...
상세 inventory, backend key, IAM, blockers는 `infra/aws/production/README.md`와 `import-manifest.json`을 따릅니다. 이 Phase에서는 live Terraform/AWS 명령을 실행하지 않습니다.
...
ECS task definition 권한은 `ecs:RegisterTaskDefinition`을
`sketchcatch-production-api:*`와 `sketchcatch-production-web:*` family ARN으로 제한합니다. `ecs:DescribeTaskDefinition`은 AWS ECS service authorization에서
resource-level permission을 지원하지 않으므로 별도 statement의 `Resource: "*"`를 유지합니다.
...
enable_ecs_worker_dispatch 기본값은 false입니다. worker role trust migration과 worker smoke 전에는 운영 API를 in_process로 유지합니다. worker mode를 활성화하면 connection setup의 caller principal도 worker task role ARN으로 바뀌므로 기존 connection은 재검증해야 합니다. worker execution role은 DATABASE_URL, REDIS_URL, CLOUDFORMATION_TEMPLATE_TOKEN_SECRET만 읽습니다.
...
### API/web warm-up과 split

1. warmup plan에서는 legacy nginx target weight 100, API/web weight 0을 유지합니다.
2. legacy ECS service, target group, port 80 security group rules에는 삭제 보호를 유지합니다.
3. warm-up apply 후 API/web service가 stable이고 두 target이 healthy인지 확인합니다.
4. production Host와 TLS SNI를 사용해 parallel ECS ALB를 direct smoke합니다.
5. 별도 split plan에서 API/web weight 100, legacy weight 0으로 전환합니다.
6. 배포 workflow의 API/web jobs는 같은 image tag를 병렬 배포하고 둘 다 service stability를 기다립니다.
7. split smoke가 통과하기 전에는 Route53을 변경하지 않습니다.

### Route53 cutover와 rollback checklist

cutover 전:

- API/web ECS desired/running count, 두 target group health, API/web/worker log를 확인합니다. legacy nginx log는 EC2 rollback 확인에만 사용합니다.
