...
현재 구현 범위에는 Destroy 자동 실행이 포함되지 않습니다. 데모 후 cleanup은 AWS 콘솔 또는 별도 절차로 확인합니다.
...
- `destroy`는 현재 MVP Apply 구현 범위에서 제외하고, 나중에 apply와 별도 승인 흐름으로 둡니다.
...
- Destroy는 현재 자동 실행하지 않으므로, 남은 리소스 정리는 사용자에게 명확히 안내합니다.
