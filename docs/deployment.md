...
SketchCatch가 사용자 Source Repository에 생성하는 ECS/Fargate CodeBuild는 registry cache나 사용자 ECR 권한을 사용하지 않습니다. 서버가 생성한 buildspec으로 API image와 frontend를 한 번 빌드·검증하고, 선택한 CodeBuild project에만 한정된 STS session과 presigned multipart URL로 SketchCatch 내부 Artifact S3에 immutable candidate를 저장합니다. API는 multipart complete 전에 실제 part 크기를 확인하고 압축 해제 파일 수·개별 파일·전체 크기를 제한합니다. 내부 Artifact bucket은 Public Access Block, 기본 암호화, TLS-only policy를 사용합니다. 현재 Vite demo frontend는 `VITE_API_BASE_URL=/`로 빌드해 CloudFront same-origin `/api/*`를 사용합니다. ECR publish와 ECS/S3/CloudFront activation은 승인 뒤 trusted worker가 별도 exact-resource 세션으로 수행합니다.
...
같은 commit과 workflow에 자동 `push`/`workflow_run` 실행과 별도 `workflow_dispatch` 실행이 함께 있으면 자동 실행을 source of truth로 선택합니다. 수동 실행이 나중에 끝났다는 이유만으로 commit 감지 결과나 자동 배포 실패를 성공으로 덮어쓰지 않습니다. GitHub가 `skipped` job의 log를 생성하지 않은 경우에는 log를 요청하지 않고 stage 상태만 저장하며, 오래된 개별 job log가 404 등으로 없어졌다면 해당 stage에 비민감 warning을 남기고 다른 stage와 run 상태의 영속화를 계속합니다. release evidence를 읽지 못한 경우에는 ApplicationRelease를 만들지 않습니다.
...
hosting bucket의 versioning이 `Enabled`가 아니거나 prefix가 이미 사용 중이면 게시하지 않는다. output과 manifest를
SHA-256 checksum으로 업로드하고 manifest `VersionId`를 기록한다. CloudFront distribution은 설정된 S3 bucket을
가리키는 정확히 하나의 origin이어야 하며, 현재 `OriginPath`를 rollback baseline으로 저장한 뒤 새 release prefix로
전환한다. distribution이 `Deployed`가 된 후 `/*` invalidation 완료와 HTTPS Output URL을 검증한다. distribution
전환, invalidation, health 검증이 실패하면 이전 `OriginPath`를 복원하고 새 invalidation 완료를 확인한다.
...
`sketchcatch-infra.yml`은 생성된 Terraform 경로가 바뀐 target branch push에서 Terraform backend S3 bucket/key를 부트스트랩하고 `terraform plan`을 실행합니다. `terraform apply` job은 GitHub Environment approval 뒤에만 실행됩니다. `sketchcatch-app.yml`은 application 경로 push 또는 수동 실행에서 SketchCatch release-run API만 호출합니다. ECS/Fargate의 실제 순서는 CodeBuild preflight → immutable candidate → trusted worker ECR/ECS → frontend S3/CloudFront → HTTPS health입니다. Lambda, EC2/ASG, static site의 기존 runtime adapter 계약은 별도 경로를 유지합니다. `sketchcatch-destroy.yml`은 수동 실행과 Environment approval 뒤 같은 S3 backend로 `terraform destroy`를 실행합니다.
...
16. full_stack이면 Apply output/resource identity와 runtime fingerprint 재검증
17. trusted worker가 API OCI archive를 ECR에 push하고 새 ECS revision task health 확인
18. frontend assets, index.html, CloudFront /* invalidation, public commit marker 순서로 활성화
19. 모든 scope별 실행이 끝난 뒤 SUCCESS 또는 PARTIALLY_FAILED 표시
20. cleanup 필요 시 terraform plan -destroy
21. 사용자 승인
22. destroy tfplan apply
23. DESTROYED 상태와 cleanup 결과 확인
```
