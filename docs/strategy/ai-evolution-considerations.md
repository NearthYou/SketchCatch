...
> 상태: PR 검토용 초안이다. MVP에서 제외한 항목을 잊지 않기 위한 참고 문서이며, 팀 전체 로드맵 확정 문서가 아니다.

이 문서는 경근 AI 파트 범위를 정리하면서 MVP 결정에서 끝내지 않고, 이후 고도화 경로와 재검토 조건을 함께 남기기 위한 문서다. MVP 문서에는 지금 구현할 범위를 적고, 이 문서에는 나중에 확장할 때 다시 볼 판단 근거를 적는다.
...
- 팀 전체 발표 흐름, 실제 Plan/Apply 노출 여부, 실패 상황을 메인 발표에 넣을지는 팀 공통 결정으로 둔다.
...
- LLM은 분류 결과, 오류 원문, 실행 단계, 관련 Resource 맥락을 받아 쉬운 설명과 다음 행동을 만든다.
...
- 실패한 Apply나 Terraform 오류를 경근 파트 문서에서 메인 발표 흐름으로 확정하는 것은 팀 공통 결정 범위를 침범하므로 하지 않는다.
...
- 실제 실패 로그가 충분히 쌓이면 팀 협의 후 오류 설명을 별도 학습/트러블슈팅 흐름으로 구성한다.
