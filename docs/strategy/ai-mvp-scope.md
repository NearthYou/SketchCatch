...
- 윤서 파트는 비용 숫자 계산, 월 예상 비용, 리소스별 비용 집계를 책임진다.
- 경근 파트는 비용 숫자의 원인이 되는 Cost Risk, 비용 산정 근거, 초보자용 수정 가이드를 설명한다.
- 경근 파트는 비용 금액의 source of truth가 아니다. 비용 설명은 윤서 파트의 계산 결과나 rule engine finding을 입력으로 받아 생성한다.
...
- 비용 금액이 표시되는 경우에는 윤서 파트가 제공한 계산 결과임을 전제로 설명한다.
...
| --- | --- | --- |
| 윤서 | 로그인 사용자, AWS 연결 상태, 비용 계산 결과, 리소스별 비용 집계 | 비용 위험 설명, 예산 초과 이유, 비용 산정 근거 설명 |
