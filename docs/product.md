...
Repository Analysis에서 Git/CI/CD 연결을 시작하면 GitHub App callback은 이미 분석한 Repository만 자동으로 연결한다. 사용자는 Repository를 다시 고르거나 다시 분석하지 않으며, `프로젝트 배포 타깃`과 `GitOps 감시 설정`을 모두 저장한 뒤 기존 추천·질문 상태로 돌아가 Board 생성을 계속한다. 초기 제품 검증 기간에는 어떤 추천 Template을 선택해도 callback 배포 타깃을 ECS Fargate로 통일하고 분석 commit SHA와 Dockerfile 근거, 프로젝트 이름으로 안전한 기본값을 채운다. 이 설정은 실제 cloud 배포나 Git 변경을 실행하지 않는다.
