...
`aws_autoscaling_group`, `aws_s3_bucket`, `aws_db_subnet_group`, `aws_api_gateway_rest_api`,
`aws_api_gateway_resource`, `aws_cloudwatch_event_rule`은 Terraform HCL `resource` identity를
유지하면서, 보드에서는 child node를 담을 수 있는 visual area node로도 동작한다. 따라서 이 리소스들의
Terraform 의미값은 `node.parameters`에 두고, 내부에 배치된 child와의 포함 관계만 child
`node.metadata.parentAreaNodeId`에 둔다.
...
신규 일반 리소스 아이콘 node의 기본 `size`는 `56x56`이다. VPC, Subnet, Security Group, Region, AZ,
Group, Auto Scaling Group, S3 Bucket, DB Subnet Group, API Gateway REST API, API Gateway Resource,
EventBridge Rule처럼 포함 관계를 표현하는 영역 node는 catalog의 별도 영역 크기를 사용하며, 일반 아이콘
축소 때문에 자동으로 절반 축소하지 않는다.
