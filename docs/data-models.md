...
Terraform Preview v1에서 Region과 Availability Zone은 화면 전용 metadata가 아니라 board-only 영역 리소스로 다룬다. 새로 생성되는 Region/AZ 영역 노드는 각각 `parameters.resourceType = "aws_region"`, `parameters.resourceType = "aws_availability_zone"`을 갖고, 선택 값은 `parameters.values.awsRegion`, `parameters.values.awsAvailabilityZone`에 저장한다. 이 두 타입은 Terraform provider schema의 resource/data block으로 렌더링하지 않고, 보드 포함 관계와 AZ-aware 리소스 보강에만 사용한다. 기존 `design_region`, `design_az`, `sketchcatch_region`, `sketchcatch_az` metadata는 저장 데이터 호환을 위해 읽을 수 있다.
...
Terraform Preview는 `aws_region` 영역 리소스를 Terraform provider block으로 렌더링하지 않는다. Region 영역 리소스는 보드의 포함 영역과 사용자 선택값을 표현하는 board-only resource이며, 서로 다른 AWS region을 선택한 Region 영역 리소스가 둘 이상 있어도 provider 충돌 오류를 만들지 않는다.
Terraform Sync는 `provider "aws" { region = ... }` 같은 provider block을 Region 영역 리소스 create/update/delete 의도로 해석하지 않고 무시한다. Terraform 입력의 resource/data block과 AZ-aware 리소스의 `availability_zone` attribute만 DiagramJson 동기화 대상으로 삼는다.
...
Terraform Sync는 `aws_subnet`, `aws_ebs_volume`의 `availability_zone` attribute를 `aws_availability_zone` 영역 리소스 create/update proposal로 되돌리고, 해당 리소스의 `metadata.parentAreaNodeId`가 AZ 영역 node id를 가리키게 한다. 이때 child resource의 직접 `availabilityZone` 값은 제거하고 AZ 영역 리소스 값으로 승격해 다음 Preview에서 같은 `availability_zone` HCL을 재생성한다.
...
Parameter panel은 현재 required main parameter만 사용자 입력 대상으로 노출한다. 단, Terraform provider가 nested block을 요구하는 리소스는 renderer가 main parameter의 짧은 입력을 HCL block 구조로 정규화할 수 있다. 예를 들어 S3 Versioning의 `status`는 `versioning_configuration.status`로, S3 기본 암호화의 `sseAlgorithm`/`kmsMasterKeyId`는 `rule.apply_server_side_encryption_by_default` block으로, S3 Lifecycle rule의 `expirationDays`는 `expiration.days` block으로 렌더링한다. EC2 root block device, Auto Scaling Group launch template/tag, DynamoDB attribute, Lambda environment, API Gateway endpoint configuration처럼 catalog가 nested-block main parameter로 저장한 값은 Terraform attribute assignment가 아니라 HCL nested block으로 출력한다. 사용자가 이미 명시 nested block 값을 가진 경우에는 그 구조를 우선한다.
...
```ts
type ReverseEngineeringScanStatus = "pending" | "running" | "success" | "failed" | "cancelled";
