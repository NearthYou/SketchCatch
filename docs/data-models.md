...
형제 간격, viewport 비율, edge 길이, 흐름·support·공백 비율의 차이를 비용으로 반영한다. 이 artifact는 29개 사용
가능 Template과 1개 unavailable evidence를 별도 생성 명령으로 검증하며, 원본 `source-exact` fixture 자체는 변경하지
않는다.
