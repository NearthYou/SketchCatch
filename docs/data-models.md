...
`웹사이트 하나 배포하고 싶어`처럼 대상은 아키텍처와 관련 있지만 화면만 필요한지, 방문자 입력/파일 업로드가 필요한지, 로그인/데이터 저장이 필요한지 알 수 없는 요구사항은 곧바로 `static_site`로 단정하지 않는다. 단, 사용자가 보조 선택에서 `api_server`, `backend_with_db`처럼 명시적인 시나리오를 골랐다면 그 선택은 부족한 자연어 단서를 채우는 힌트로 사용한다. 보조 선택도 `auto`이거나 여전히 부족하면 Workspace AI는 전문 용어 대신 쉬운 질문과 추천 답안을 차례로 보여주고, 답변을 모아 구현 리스트를 확인받은 뒤 사용자가 진행을 승인할 때만 `ArchitectureDraft`를 요청한다. 질문에서는 `S3`, `EC2`, `RDS`, `IAM` 같은 내부 리소스 이름을 먼저 묻지 않고, 비용 영향과 보호 범위를 사용자 언어로 설명한다.

보조 선택값인 `budgetLevel`, `trafficLevel`, `securityPriority`는 단순 설명용 메타데이터가 아니라 Architecture Draft 생성 조건이다. 같은 Requirement Prompt와 같은 보조 선택값은 항상 같은 `ArchitectureJson`을 반환해야 하며, 보조 선택값이 달라지면 지원 가능한 리소스의 config가 달라져야 한다. 예를 들어 낮은 예산/작은 트래픽은 작은 EC2/RDS 크기와 낮은 로그 보존 기간을 쓰고, 보통 트래픽/높은 보호 조건은 더 큰 인스턴스, 더 긴 로그 보존 기간, 공개 접근 차단 같은 안전한 기본값을 반영한다.
