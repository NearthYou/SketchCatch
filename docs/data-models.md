...
`POST /api/projects/:projectId/aws-connections/:connectionId/test`는 저장 없이 연결 테스트만 수행한다. 요청 body에서는 `roleArn`만 받고, `externalId`와 `region`은 DB에 저장된 connection metadata를 사용한다. STS `AssumeRole`을 호출하고, 반환된 임시 credential로 STS `GetCallerIdentity`를 호출한다. 그 뒤 같은 role을 `externalId` 없이 다시 assume해 보고, 성공하면 trust policy가 잘못 열린 것으로 보고 거부한다. 응답에는 `ok`, `accountId`, `callerArn`, `region`만 포함한다. 임시 credential의 `accessKeyId`, `secretAccessKey`, `sessionToken`은 DB, API 응답, 프론트 상태에 넣지 않는다.
...
Terraform init은 deployment에 저장된 `awsConnectionId`로 verified AWS connection을 조회한 뒤 매번 verified `roleArn`과 `externalId`로 다시 `AssumeRole`하고, externalId 없이도 role이 assume되는지 다시 확인한다. 받은 임시 credential은 Terraform child process env로만 전달하며, Terraform child process는 allowlist 기반 환경 변수와 임시 AWS credential만 상속한다. plan/apply가 연결되면 같은 계약을 사용하되, apply 전에는 승인 당시 `accountId`, `region`, `tfplanHash`와 실행 시점 값을 다시 비교해 drift가 있으면 차단한다.
...
| `AwsConnection` | `aws_connections`, `/api/projects/:projectId/aws-connections`, `/api/projects/:projectId/aws-connections/:connectionId/cloudformation-template`, `/api/aws/connections/cloudformation-template`, `/api/projects/:projectId/aws-connections/:connectionId/test`, `/api/projects/:projectId/aws-connections/:connectionId/verify` | Role Assume 설정값 제공, CloudFormation Role 생성 템플릿 제공, connection-scoped STS 연결 테스트, 저장형 verify, deployment init credential 주입 구현됨 |
