...
Repository Analysis는 저장소 evidence로 배포 방식과 CI/CD 사용 기본값을 추론하되, 사용자가 한 화면에서 수정할 수 있어야 한다. 배포 방식은 `ec2_vm`, `container`, `serverless` 중 하나만 선택하며, UI 표기는 `EC2/VM based`, `container based`, `serverless based`로 한다. CI/CD 여부는 checkbox로 입력한다.
