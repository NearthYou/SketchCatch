...
Repository Analysis는 저장소 evidence로 배포 방식과 CI/CD 사용 기본값을 추론한다. Container, Serverless, EC2/VM처럼 배포 방식이 명확한 evidence가 있으면 Template 후보 선택과 중복되는 배포 방식 입력을 숨기고, 사용자가 최종 선택한 Template에서 유효 배포 방식을 계산한다. 배포 evidence가 없거나 모호할 때만 같은 화면에서 `ec2_vm`, `container`, `serverless` 중 하나를 선택하게 하며, UI 표기는 `EC2/VM 기반`, `컨테이너 기반`, `서버리스 기반`으로 한다. CI/CD 여부는 checkbox로 입력한다. 공개 URL 분석 흐름은 Template과 CI/CD 설정을 확인한 뒤 별도 추가 질문 단계로 전환한다. GitHub App 저장소 연결 구역은 추가 질문 단계에서 CI/CD 인계를 선택했고 연결된 Source Repository가 없을 때만 표시하며, 새 URL 분석을 시작하면 해당 선택을 초기화한다.
