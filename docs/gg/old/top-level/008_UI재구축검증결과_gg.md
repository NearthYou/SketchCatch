...
승인 hash를 계산하는 JSON은 안전 검사 입력으로 사용하지 않는다. 안전 검사는 bundle 안의 모든 `.tf` 원문을 따로 꺼내 이어서 검사하므로, 두 번째 파일에 금지된 module이나 설정을 숨겨도 승인을 거절한다.

Lambda처럼 Apply 전에 로컬 archive 파일을 만들어야 하는 Resource도 같은 Terraform 원문을 기준으로 찾는다. 다중 파일 bundle이어도 필요한 준비 단계를 건너뛰지 않는다.
...
- 다중 Terraform hash 입력과 안전 검사 입력 분리
