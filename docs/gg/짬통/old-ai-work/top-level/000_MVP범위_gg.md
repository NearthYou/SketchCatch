...
지원하지 않는 말이 들어오면 지원 가능한 부분만 초안으로 만들고 경고를 남긴다.

지원 가능한 단서나 대체 가능한 단서가 전혀 없으면 초안을 만들지 않고, 사용자가 요구사항을 더 구체적으로 쓰도록 안내한다.

```text
요구사항에서 명확한 아키텍처 단서를 찾지 못해 초안을 생성하지 않았습니다.
예: 웹사이트 하나 배포하고 싶어, 파일 업로드 페이지가 필요해
```
...
점수가 모두 0이고 대체 가능한 지원 범위 밖 단서도 없으면 초안을 만들지 않는다.
