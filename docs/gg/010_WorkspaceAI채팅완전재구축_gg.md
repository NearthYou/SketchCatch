...
각 대화는 별도의 내역, 입력 중인 문장, 요청 처리 상태, 오류, 승인 대기 상태를 유지한다. 화면에서는 `초안 제안`, `AI 오류`, `Preview 설명` 명칭을 사용하지 않는다.

일반 런처는 마지막으로 사용한 대화를 연다. Terraform Issue와 Terraform Preview에서 진입한 경우만 각각 `오류 분석`과 `에이전트 리뷰`를 강제로 연다. 설계 제안의 기준 Board가 달라진 경우에는 제안을 지우지 않고 `오래된 제안`으로 남기되, 적용 버튼을 비활성화하고 다시 생성만 허용한다.
