...
> 상태: 코치 피드백을 반영한 gg AI 파트 PR 검토용 초안이다. 실제 구현 전 shared type, API schema, 팀원 파트 계약에 한 번 더 맞춘다.
...
→ Architecture Board / Terraform Preview / Plan-Apply 화면에 표시
```

AI는 설계 의도 해석과 설명을 돕지만, Terraform 최종 생성, Apply 판단, AWS 권한 변경, 실제 배포 실행의 최종 결정자가 아니다.

## LLM 적용 범위

MVP에서 LLM은 자연어 요구사항 해석, 설명 생성, 오류 설명을 맡는다. LLM 응답은 항상 검증 대상이며, 검증을 통과한 구조화 결과만 제품 흐름에 반영한다.
...
사용자가 자연어로 원하는 조건을 입력하면 Architecture Board가 열 수 있는 Practice Architecture 초안을 만든다.
...
GitHub 링크 기반 초안 생성은 MVP 메인 챌린지에서 내린다.
...
- MVP 비용 추정은 실제 청구액 보장이 아니라 Resource type, instance class, 사용 시간, traffic assumption에 기반한 추정값이다.
...
- Cost Risk, Security Risk, missing configuration, permission concern, performance bottleneck을 구분한다.
...
성능 시뮬레이션은 MVP 코드 타입이 확정되기 전까지 별도 DTO로 바로 고정하지 않고, 후속 이슈에서 `AiDesignSimulationResult` 같은 이름으로 분리한다.

## 5. 오류 설명 / 체크리스트 생성

Plan 또는 Apply에서 나온 오류와 배포 전 확인 항목을 사용자 언어로 바꾼다.
...
4. Week 4: 비용·트래픽 시뮬레이션 최소 모델, Terraform Preview 설명, Plan/Apply 오류 설명 연동
...
6. Terraform validate, Plan, Apply 오류가 발생했을 때 원인 후보와 다음 행동을 설명한다.
