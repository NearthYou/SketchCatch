...
| `bucket-policy` | S3 Bucket Policy | 920, 680 | root | S3 하단 정책 |

`viewport = { x: 0, y: 0, zoom: 0.75 }`. `distribution → bucket`은 right→left, 설정 edge는 vertical/diagonal smoothstep handle을 사용한다.
...
| `permission` | Lambda Permission | 1,360, 400 | root | Compute 상단 support |
...
| ID | Resource | 목표 좌표 | Parent | 역할 |
| --- | --- | --- | --- | --- |
| `vpc` | VPC | 160, 160 | root | outer container |
...
| `public-route-table` | Public Route Table | 320, 500 | `vpc` | Public row rail |
| `public-route-a` / `public-route-b` | Route Associations | 520, 500 / 520, 580 | `vpc` | Public rail helper |
| `public-subnet-a` / `public-subnet-b` | Public Subnets | 680, 440 / 1,440, 440 | `vpc` | A/B public row containers |
| `load-balancer` | Load Balancer | 1,240, 560 | `vpc` | Public runtime |
| `listener` | ALB Listener | 1,360, 560 | `vpc` | ALB 근접 support |
| `target-group` | Target Group | 1,480, 680 | `vpc` | ASG 진입점 |
| `alb-security-group` | Security Group | 860, 640 | `vpc` | ALB 근접 security |
| `app-route-table` | App Route Table | 320, 820 | `vpc` | App row rail |
| `app-route-a` / `app-route-b` | Route Associations | 520, 820 / 520, 900 | `vpc` | App rail helper |
| `app-subnet-a` / `app-subnet-b` | App Subnets | 680, 760 / 1,440, 760 | `vpc` | A/B app row containers |
| `application-group` | Auto Scaling Group | 1,120, 860 | `vpc` | App runtime container |
| `launch-template` | Launch Template | 1,320, 960 | `application-group` | ASG 내부 helper |
| `latest-ami` | AMI data | 720, 1,260 | `vpc` | 하단 support |
| `app-security-group` | Security Group | 920, 1,020 | `vpc` | App security |
| `db-route-table` | DB Route Table | 320, 1,140 | `vpc` | DB row rail |
| `db-route-a` / `db-route-b` | Route Associations | 520, 1,140 / 520, 1,220 | `vpc` | DB rail helper |
| `db-subnet-a` / `db-subnet-b` | DB Subnets | 680, 1,080 / 1,440, 1,080 | `vpc` | A/B DB row containers |
| `db-subnet-group` | DB Subnet Group | 1,360, 1,240 | `vpc` | DB support |
| `database` | RDS Instance | 1,680, 1,160 | `vpc` | DB runtime |
| `db-security-group` | Security Group | 1,600, 1,320 | `vpc` | DB security |
...
| --- | --- | --- | --- | --- |
| `vpc` | VPC | 160, 160 | root | outer container |
| `alb-security-group` | Security Group | 680, 280 | `vpc` | ALB 근접 security |
| `subnet-a` / `subnet-b` | Public Subnets | 400, 360 / 1,120, 360 | `vpc` | A/B network containers |
| `load-balancer` | Load Balancer | 920, 440 | `vpc` | ingress main flow |
| `listener` | ALB Listener | 960, 600 | `vpc` | ALB 근접 |
| `target-group` | Target Group | 1,200, 600 | `vpc` | service 진입점 |
| `cluster` | ECS Cluster | 440, 760 | `vpc` | workload container |
| `service` | ECS Service | 800, 980 | `cluster` | workload main flow |
| `task-security-group` | Security Group | 600, 1,160 | `vpc` | service 근접 security |
| `internet-gateway` | Internet Gateway | 320, 1,300 | `vpc` | footer support |
| `route-table` | Route Table | 520, 1,300 | `vpc` | footer support |
| `route-a` / `route-b` | Route Associations | 720, 1,300 / 880, 1,300 | `vpc` | footer helper |
...
| `log-group` | CloudWatch Log Group | 2,200, 1,160 | root | Definition/Ops rail |

`viewport = { x: 0, y: 0, zoom: 0.40 }`. `cluster → service`은 container 내부, `service → task`는 VPC에서 rail로, repository / task / log / role 관계는 rail 안에서 위→아래로 routing한다.
...
`viewport = { x: 0, y: 0, zoom: 0.42 }`. `cluster → namespace`와 `namespace → deployment`는 containment, `deployment → service`는 right→left main workload edge, IAM authorization은 right support rail → cluster로 routing한다.
...
3. API Gateway, ECS Cluster, EKS Cluster, Kubernetes Namespace를 기존 실제 resource node의 visual container로 인식한다. 새 node를 만들지 않는다.
