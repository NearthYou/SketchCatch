...
- Template의 역할별 표시 label은 기존 Template 정의를 유지하되, node type·kind·icon은 반드시 해당 Catalog resource에서 생성한다. 즉, `Public Subnet A` 같은 역할 label은 Terraform logical name이나 임시 노드가 아니다.
...
`viewport = { x: 0, y: 0, zoom: 0.64 }`. runtime은 right→left, IAM은 Lambda bottom→top으로 routing한다. `permission`은 Lambda 상단 support로만 배치하며, 기존 관계 불변조건 때문에 새 edge는 추가하지 않는다.
...
`viewport = { x: 0, y: 0, zoom: 0.38 }`. VPC → subnet `contains` stays grouping-only. `load-balancer → application-group → database`는 Public → App → DB tier를 따라 위에서 아래로 이어지는 기존 runtime relation이며, 두 edge는 bottom→top handle을 사용한다.
