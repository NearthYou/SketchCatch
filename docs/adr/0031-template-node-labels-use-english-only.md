...
초기 Template label은 node를 구분하거나 구조를 설명하지 않는 Template 전체·도메인·환경·구현 상세·provider 맥락 수식어를 생략한다. 예를 들어 `Items`와 단일 API Stage의 `Production API Stage`에서 `Production`은 설명력을 더하지 않는 filler이므로 각각 `Items`, `API Stage`로 사용한다.
