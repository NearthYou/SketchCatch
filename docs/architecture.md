...
- `apps/web`: Architecture Board, IaC Preview, deployment check, Deployment 화면
...
| Runtime Cache | Redis                                | Deployment, Reverse Engineering, CI/CD 상태와 15분 Live Observation 세션·집계 보조 |
...
| CI/CD handoff와 상태 추적                  | `apps/api` 또는 ECS RunTask worker                  | 승인 없는 commit/apply, secret 저장                                  |
| Runtime Cache 사용                             | `apps/api` 또는 ECS RunTask worker                  | 사용자 infrastructure resource로 노출                         |
...
음성 Requirement Input은 Amazon Transcribe로 전사한 뒤 사용자 확인을 거쳐 Requirement Prompt가 된다. AI, Bedrock, Amazon Q Assistance는 추천과 설명을 보강하지만 ?명봽???ㅺ퀎, IaC Preview, Git 변경, Deployment 실행을 사용자 수락 없이 변경하지 않는다.
...
Board Auto Arrange Adapter는 이 넓은 Compiler 결과에서 geometry, edge routing, 소속 없는 Presentation Frame만
허용한다. Presentation Frame은 `design` node의 제목·위치·크기·스타일만 가지며 Resource ID 목록이나 부모
영역 값을 갖지 않는다. 자동 생성 Frame은 Design kind, Design Group type, catalog identity, `board-auto-frame:` ID
prefix가 모두 맞는 full tuple로 식별한다. full tuple이 맞고 잠기지 않은 Frame만 자동 병합·삭제한다. 표시 Frame은
drop parent, containment 추론, 실제 area auto-size에서 제외한다. 사용자 작성 Design Group은 geometry 제안 대상일 수
있지만 자동 소유권으로 바꾸거나 제거하지 않는다. Adapter는 후보의 시각값만 원본 의미 위에 다시 조립한 뒤 Resource의
`parentAreaNodeId`를 포함한 containment, 관계, 설정, Terraform 의미가 입력과 같음을 확인하고, 다르면 후보를 제외한다.
일반 Diagram 편집 경로는 주변 Resource 이동을 감시해 Presentation Frame을 자동 수정하지 않는다. Frame 재계산은
사용자가 Board Auto Arrange를 다시 요청했을 때만 실행되고, 새 미리보기와 User-Accepted Change 경계를 거친다.
Board Auto Arrange Adapter는 의미 보존 검사와 시각 diff 존재 여부만 결과 제공 gate로 사용한다. 원본은 비교
baseline으로 유지하고 후보 점수 경쟁에서는 제외한다. 서로 다른 안전한 constrained Diagram이 여러 개면 geometry와
edge route가 같은 결과를 deduplicate한 뒤 최대 3개 후보를 반환한다. Compiler의 quality score나 layout finding 개선
여부는 후보를 숨기거나 적용 버튼을 막지 않으며, 각 최종 constrained Diagram을 기준으로 다시 계산해 후보 순서와
사용자 설명에만 사용한다. 데스크톱은 thumbnail 갤러리에서 선택한 후보를 원본과 나란히 비교하고, 모바일은 가로
gallery 뒤 같은 viewport의 원본/후보를 전환한다. 후보 전환은 저장 경계를 통과하지 않는다.
선택한 후보 적용은 생성 당시 Diagram fingerprint와 ProjectDraft expected revision을 함께 검증하고 서버의 revision
compare-and-swap을 통과할 때만 단일 History write를 만든다.

## SketchCatch production infrastructure 관리 경계

SketchCatch 자체 production infrastructure Terraform은 사용자가 만드는 ?명봽???ㅺ퀎와 managed deployment/CI/CD Deployment state에서 분리합니다.
...
AWS 연결 권한 변경은 환경설정이 소유하고 Reverse Engineering은 조회 실패를 감지해 복구 화면으로 안내만 합니다. 모든 기존 verified 연결은 원래 CloudFormation Stack과 Role을 그대로 유지하고, 연결별 Import Access Stack Pair에서 Reverse Engineering 권한만 관리합니다. Manager Stack은 CloudFormation service Role, 최소 제어 Policy, 읽기 전용 정리 확인 Policy를 소유하고, Policy Stack은 기존 연결 Role에 붙는 읽기 Managed Policy만 소유합니다. service Role은 CloudFormation만 Assume하며 연결·조회·배포에는 사용하지 않습니다. 기존 배포 Policy 전체를 다시 적용하거나 새 connection을 만들지 않으며, 사용자의 AWS 승인 전에는 어떤 권한도 변경하지 않습니다.

AWS 승인 뒤에는 같은 Role로 bounded read probe를 실행합니다. Role Assume과 Stack 상태는 진단 정보일 뿐 사용자 성공 조건이 아닙니다. EC2 기반 네트워크·compute, S3, RDS, Load Balancer, ECS, CloudFront 주요 reader가 성공하면 가져오기를 허용합니다. Resource Explorer, Tagging API, IAM, KMS, CloudWatch·Logs, API Gateway, Lambda, AMI 확장 reader 실패는 부분 제한으로 분류하고 주요 가져오기를 막지 않습니다. probe는 write API를 호출하지 않고, 실패해도 배포에 사용하는 AWS 연결의 `verified` 상태를 내리지 않습니다. 주요 권한 부족, 확장 조회 제한, Resource Explorer 미설정, 일시 provider 오류는 서로 다른 안전한 상태로 분류합니다.

환경설정은 짧은 수명의 Template URL로 정확한 Manager Stack Quick Create 화면을 열고 사용자가 AWS Console에서 승인하게 합니다. API는 Manager Template hash, service Role trust·권한, 제어·확인 Policy와 attachment까지 검증합니다. 사용자가 추가될 읽기 범위를 한 번 승인하면 Settings API가 service Role ARN과 서버 소유 immutable Template URL을 지정해 정확한 Policy Stack을 생성하거나 갱신합니다. `TemplateBody`, 다른 connection의 Stack·Template·Role, 다른 Resource type은 허용하지 않습니다. 사전 Change Set은 만들지 않습니다. 승인 정보는 connection, 정확한 Stack pair, 기존 Role, service Role, 현재·목표 Template version과 Policy hash에 묶고 실행 직전 다시 확인합니다. 이 mutation 경계는 connection에 기록된 Policy Stack에만 묶이며 원래 연결 Stack, Role Resource, 배포 Policy를 변경하지 않습니다.

연결을 삭제할 때 환경설정은 Policy Stack과 Manager Stack을 구분해 보여주고 정확한 AWS Console 삭제 화면을 순서대로 엽니다. 사용자가 Policy Stack을 먼저 삭제하고 API가 정확한 Stack과 읽기 Managed Policy의 제거를 확인한 뒤에만 Manager Stack 삭제로 이동합니다. Manager Stack의 읽기 전용 확인 Policy는 다른 Manager Resource보다 마지막에 제거됩니다. 마지막에는 STS identity가 유효한 상태에서 service Role·제어 Policy·확인 Policy 제거를 확인합니다. 하나라도 남아 있거나 결과가 불확실하면 비활성 cleanup retry 기록으로 남깁니다. 이 정리 경로는 기존 연결 Role, 원래 연결 Stack, 기존 배포 Policy를 삭제 대상으로 포함하지 않습니다.

사용자의 각 삭제 완료 입력은 cleanup 재확인의 시작 신호일 뿐 성공 증거가 아닙니다. API는 정확한 Stack ID, Managed Policy, service Role, 제어·확인 Policy처럼 SketchCatch가 소유한 artifact를 기준으로 제거를 확인합니다. bounded import read probe도 안전 신호로 남기지만 다른 Policy가 같은 권한을 주는 경우에는 소유 artifact 정리를 막지 않습니다. 결과가 일시 오류·Role 접속 실패로 불확실하면 metadata와 원 connection row를 삭제하지 않고 cleanup retry를 유지합니다. 이 확인은 배포 연결의 `verified` 상태와 분리됩니다.
...
Redis는 Deployment, Reverse Engineering, CI/CD Integration처럼 오래 걸리는 workflow 상태와 streaming-friendly metadata를 보조한다. Live Observation은 예외적으로 15분 세션, public token SHA-256 lookup, receipt dedup, 원자 count, 1초 bucket을 Redis에만 저장하며 영구 Deployment 기록으로 승격하지 않는다. Redis 데이터는 원천 기록이 아니며, 최종 기록은 RDS/S3에 남긴다.
...
flowchart LR
  ...
  IaC --> Check["deployment check"]
  IaC --> Artifact["TerraformArtifact\nS3 object"]
  Artifact --> Direct["managed deployment"]
  Artifact --> Git["CI/CD delivery"]
...
Application artifact 계층은 `application_artifacts`의 project-scoped fingerprint와 heartbeat로 갱신하는 build claim/lease로 managed deployment와 CI/CD의 중복 build를 조정한다. 재사용 전 provider adapter가 존재 여부, exact digest, account, region, 승인된 namespace/reference, ownership scope를 read-only로 다시 확인한다. RDS row나 Redis hit만으로 재사용하지 않으며 검증 실패는 artifact를 `invalid`로 전환하고 정상 build로 fallback한다. release는 nullable `artifactId`로 artifact identity와 분리해 v1 evidence와 legacy record를 유지하고, 복합 `(artifactId, projectId)` FK로 cross-project 연결을 차단한다.
...
- deployment check는 비용/보안/설정 위험을 반환한다.
...
- CI/CD Integration API는 Source Repository 연결, Terraform handoff, PR 생성, pipeline 상태 추적 흐름으로 확장한다.
...
- Reverse Engineering API는 Provider Adapter를 통해 기존 cloud Resource를 스캔하고 ?명봽???ㅺ퀎와 import suggestion을 반환한다.
...
Workspace는 managed deployment 실행 modal과 독립 `Delivery` panel을 구분한다. Direct 화면은 `Deployment`, Plan, Terraform Output과 Deployment log를 읽고, Delivery는 프로젝트 GitHub 연결, Source Repository, monitoring, 배포 타깃, readiness와 GitHub workflow run별 `GitCicdPipelineRun`을 함께 보여준다. 배포 modal의 CI/CD tab은 조회 요약과 `Delivery 열기`만 제공하며 같은 설정 editor를 중복 소유하지 않는다. 같은 commit SHA의 App·Infra run도 서로 다른 실행 기록이다. 이 UI 선택은 Deployment나 Pipeline Run 상태를 변경하지 않는다.

CI/CD 관측의 영구 source of truth는 RDS다.
...
- `handoff_id`는 승인된 CI/CD handoff와 연결할 때만 사용하며, 기존 handoff record를 Pipeline Run으로 변환하지 않는다.
...
AWS Resource나 Source Repository를 변경하지 않으며, CI/CD handoff 승인이나 PR 생성도 수행하지
않는다. S3 또는 RDS evidence를 일시적으로 읽지 못하면 준비 항목을 완료로 추측하지 않고 503으로
응답한다.

CI/CD handoff가 승인된 Apply Plan을 다시 검증할 때 S3 object missing/404 또는 SHA-256 불일치만
...
완료 알림의 source of truth는 RDS Inbox와 idempotent outbox다. managed deployment와 GitOps Pipeline Run의
...
MVP는 AWS Provider Adapter 기준이다. `Resource`, `?명봽???ㅺ퀎`, `InfrastructureGraph`, `Reverse Engineering`은 provider-neutral 모델을 유지하고, provider별 차이는 adapter에 둔다. 장기적으로는 아래처럼 확장한다.
...
### 결정 7: Repository build와 application activation의 실행 권한을 분리한다
...
최초 ECS/Fargate API·frontend 활성화는 managed deployment가 담당한다. CI/CD handoff API는 승인된 인프라 Apply 증거와 현재 target에 일치하는 성공 Direct `ApplicationRelease`를 같은 서버 predicate로 다시 확인한 뒤에만 설치 PR provider를 호출한다. 설치 PR은 후속 push용 Workflow와 설정만 추가하며, PR merge 자체는 최초 앱 릴리즈나 AWS mutation을 시작하지 않는다.
...
| 영역 | 현재 결정 |
| --- | --- |
| AI와 안전 | AI는 요구사항 해석과 설명을 보조하지만 graph validation, IaC 생성, 안전 검사는 결정론적 계약을 따른다. 상태 변경은 항상 사용자가 수락해야 한다. |
| 실행 경로 | managed deployment와 CI/CD delivery를 분리하고 두 경로 모두 Plan, deployment check, 승인, secret masking, cleanup 경계를 유지한다. |
| Provider | MVP는 AWS-first지만 `Resource`, `?명봽???ㅺ퀎`, `InfrastructureGraph`, Reverse Engineering은 Provider Adapter 기반의 provider-neutral 모델을 유지한다. |
| Repository Analysis | repository tree와 제한된 evidence를 정적으로 분석하며 임의 Source Repository 코드를 실행하지 않는다. monorepo도 하나의 분석과 여러 Application Unit으로 표현한다. |
| Template Selection | 저장소 전체에 맞는 Template 하나와 선택 근거를 전달한다. 안전하게 표현할 수 없으면 억지로 선택하지 않고 불일치 이유를 반환한다. |
| 분석 저장 | repository 원문은 저장하지 않고 마지막 분석 요약, revision, 선택 결과만 보존한다. 재분석 결과가 최신 상태를 대체한다. |
| AI Handoff | AI는 확정된 Template Selection을 임의로 교체하지 않고, 감지되지 않은 요구사항과 사용자 입력을 보완한다. |
| Board Compiler | Compiler 제안은 Preview에서 검토하며, Board Auto Arrange는 Resource·관계·설정을 유지하고 위치와 연결선만 바꾼다. |
| Workspace AI | 대화와 AI 결과는 AI 채팅이 소유한다. 장식 표현은 진행률이나 실제 Resource로 간주하지 않고 Compiler 성공 결과만 Preview로 공개한다. |
| AI provider | AWS credit 모드에서는 Amazon Q, Bedrock, deterministic fallback 순서로 설명 provider를 선택하며 실제 적용 계약은 provider와 분리한다. |
| Live Observation | provider-neutral snapshot으로 최대 세 개의 근거 기반 신호를 보여준다. 추측을 사실로 바꾸거나 자동으로 Deployment·AWS Resource를 수정하지 않는다. |

새로운 장기 결정은 별도 문서를 만들기보다 이 섹션과 관련 canonical 문서를 함께 갱신한다. 과거 결정의 상세 배경은 Git history에서 확인한다.
