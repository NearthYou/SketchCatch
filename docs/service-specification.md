# SketchCatch Service Specification

이 문서는 기획자와 개발자가 SketchCatch를 같은 서비스 단위로 이해하기 위한 Service Specification이다. `docs/product.md`, `docs/data-models.md`, `docs/architecture.md`, `docs/deployment.md`를 읽기 쉬운 실행 계획으로 풀어쓰며, 계약이 충돌하면 canonical 문서를 우선한다.

## 1. 서비스 정의

SketchCatch는 자연어/음성 요구사항, Source Repository, 기존 cloud state를 입력으로 받아 provider-neutral ?명봽???ㅺ퀎를 만들고, 이를 Architecture Board, IaC Preview, deployment check, managed deployment, CI/CD delivery, Reverse Engineering, Deployment History, Auto Cleanup까지 연결하는 multi-cloud-ready IaC 운영 서비스다.

MVP 구현은 AWS-first, Terraform-first로 간다. 그러나 제품 설명과 도메인 모델은 AWS-only가 아니다. AWS는 첫 번째 Provider Adapter이며, Azure/GCP 같은 다른 provider는 같은 `Resource`, `?명봽???ㅺ퀎`, `InfrastructureGraph`, `Provider Adapter` 경계를 확장해 붙인다.
...
| 실제 배포는 실패와 비용 사고가 무섭다 | Terraform apply 실패, public 접근, 남은 리소스 비용이 걱정된다 | deployment check, Deployment Safety Gate, 승인된 Plan, Deployment History, Auto Cleanup을 제공한다 |
| 기존 cloud state를 이해하기 어렵다 | 이미 만들어진 AWS 리소스가 어떤 서비스 구조인지 파악하기 어렵다 | Provider Adapter 기반 Reverse Engineering으로 ?명봽???ㅺ퀎와 import suggestion을 만든다 |
| 팀 운영 흐름과 개인 실습 흐름이 다르다 | 빠른 검증은 직접 해보고 싶고, 운영 배포는 PR/CI/CD로 관리해야 한다 | managed deployment와 CI/CD delivery를 둘 다 제공한다 |
...
| 플랫폼/DevOps 엔지니어 | 기존 cloud state 분석, Terraform 전환, 운영 배포 흐름 정리가 필요함 | Reverse Engineering, import suggestion, CI/CD handoff, 위험 요약을 한곳에서 확인한다 |
...
| 영역 | 현재 상태 | 다음 구현 방향 |
| --- | --- | --- |
| 인증/사용자/프로젝트 | 회원가입, 로그인, OAuth, password reset, refresh token, 프로젝트 CRUD, project draft, asset upload 기반이 있다 | 프로젝트별 서비스 여정에서 Source Repository, Reverse Engineering, CI/CD 상태를 연결한다 |
...
| AWS 연결 | AWS Role 연결 metadata, CloudFormation template/Quick Create URL, STS 검증 흐름이 있다 | CI/CD와 Reverse Engineering에서도 Provider Adapter credential 경계로 재사용한다 |
| managed deployment | Deployment 생성, init, plan, approval, apply, destroy plan, destroy, cancel, logs, log stream, outputs, deployed resources, retention 기반이 있다 | High Security Risk gate와 Cost Risk gate를 Deployment 승인 화면에 더 강하게 연결한다 |
| deployment check | AI/rule 기반 비용, 보안, 설정 finding과 checklist 기반이 있다 | High는 차단, Medium/Low는 경고/명시 승인으로 정책을 고정한다 |
| CI/CD delivery | 제품 방향과 데이터 모델이 확정되어 있다 | Source Repository 연결, Terraform PR 생성, pipeline template/status tracking, handoff record를 구현한다 |
| Reverse Engineering | Provider Adapter 기반 방향과 데이터 모델이 확정되어 있다 | AWS adapter부터 기존 Resource scan, ?명봽???ㅺ퀎 복원, import suggestion을 구현한다 |
...
| Redis Runtime Cache | 내부 인프라 역할이 확정되어 있다 | Deployment, Reverse Engineering, CI/CD 상태 추적과 로그 streaming 보조에 사용한다 |
...
5. managed deployment와 CI/CD delivery는 선택지 경쟁이 아니라 다른 사용 맥락이다.
6. Reverse Engineering은 provider-adapter 기반이다. MVP는 AWS adapter부터 구현하지만 개념은 AWS resource list가 아니다.
7. Redis는 내부 Runtime Cache다. 사용자 infrastructure resource로 제공하지 않는다.
...
flowchart LR
  ...
  Reverse --> Imported["Imported ?명봽???ㅺ퀎 Draft"]
  ...
  Gate --> Direct["managed deployment"]
  Gate --> Git["CI/CD delivery"]
...
| CI/CD handoff | 운영 배포는 PR과 pipeline으로 검토한다 | Terraform artifact, PR URL, pipeline run URL, status를 `GitCicdHandoff`로 기록한다 | 사용자는 PR 생성, pipeline 상태, 실패 원인을 SketchCatch에서 확인한다 |
...
### 7.6 deployment check와 Deployment Safety Gate
...
여기서 Redis public 접근은 SketchCatch 내부 Runtime Cache 또는 향후 provider-managed cache가 외부에 노출되는 위험을 뜻한다. MVP에서 Redis를 사용자가 보드에 올리는 infrastructure resource로 제공한다는 의미가 아니다.

### 7.7 managed deployment

managed deployment는 빠른 검증, sandbox, practice run을 위해 SketchCatch가 Terraform 실행을 관리하는 경로다.
...
CI/CD delivery는 운영 배포와 팀 리뷰를 위한 경로다. SketchCatch는 IaC Preview를 Source Repository에 넘기고, 외부 pipeline 상태를 추적한다.

| 기능 | 기획 의도 | 개발 기준 | 완료 기준 |
| --- | --- | --- | --- |
| Terraform PR 생성 | 인프라 변경을 코드 리뷰 대상으로 만든다 | Terraform artifact를 branch/commit/PR로 handoff한다 | PR에는 IaC Preview, Plan summary, deployment check, Cost Analysis 요약이 연결된다 |
...
Reverse Engineering은 기존 cloud state를 읽어 ?명봽???ㅺ퀎로 복원하고 IaC 전환 경로를 제안하는 기능이다.
...
| ?명봽???ㅺ퀎 복원 | resource list가 아니라 서비스 구조를 만든다 | resource identity, relationship, region, dependency를 그래프로 변환한다 | 사용자는 복원된 Architecture Draft를 확인한 뒤 보드에 반영한다 |
...
| 시점 | 보여줄 것 |
| --- | --- |
| ?명봽???ㅺ퀎 | 선택한 Resource 조합의 월 비용 추정, 비용 drivers, 과한 Resource 경고 |
...
3. CI/CD pipeline status polling/cache
...
| 화면/패널 | 사용자 목적 | 핵심 상태 |
| --- | --- | --- |
| Project 목록/상세 | 작업할 ?명봽???ㅺ퀎를 찾고 생성한다 | 프로젝트, 최신 draft, 최근 Deployment 상태 |
...
| deployment check | 비용/보안/설정 위험을 확인한다 | `CheckFinding`, checklist, suggestions |
| Deployment Panel | managed deployment를 실행하고 기록을 확인한다 | Deployment, plan summary, approval state, logs, resources, outputs |
| CI/CD Panel | PR과 pipeline 상태를 본다 | GitCicdHandoff, PR URL, pipeline status |
...
| 3. Deployment + CI/CD Integration + Runtime Cache | managed deployment, Git handoff, pipeline status, Redis Runtime Cache | Deployment API/UI, PR handoff, pipeline tracker, log/status streaming | `Deployment`, `DeploymentPlanArtifact`, `DeploymentLog`, `GitCicdHandoff` |
...
- CI/CD와 Reverse Engineering은 모두 provider credential과 secret을 UI에 노출하지 않아야 한다.
...
15. 운영 경로가 필요한 경우 CI/CD delivery로 Terraform PR과 pipeline handoff를 만든다.
16. 기존 cloud state에서 시작하는 경우 Reverse Engineering scan으로 ?명봽???ㅺ퀎와 import suggestion을 만든다.
...
| 기준 | 성공 신호 |
| --- | --- |
| 서비스 흐름 | Requirement Input, Source Repository, Reverse Engineering 중 하나에서 시작해 ?명봽???ㅺ퀎, IaC Preview, deployment check, 실행/hand off까지 이어진다 |
...
| 협업성 | CI/CD Path에서 PR과 pipeline 상태를 서비스 안에서 추적할 수 있다 |
...
| CI/CD | PR handoff record, pipeline status polling/cache, secret masking |
...
2. deployment check의 High/Medium/Low 정책을 Deployment approval과 연결한다.
...
6. CI/CD Handoff를 Source Repository, PR, pipeline status 단위로 구현한다.
