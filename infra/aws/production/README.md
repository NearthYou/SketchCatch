...
이 디렉터리는 SketchCatch 자체 production infrastructure를 Terraform으로 관리하기 위한 Phase 9 경계입니다. 사용자가 SketchCatch에서 실행하는 IaC Preview/Plan/Apply와 완전히 별개이며, product API나 ECS worker가 이 state를 읽거나 변경하지 않습니다.

Phase 9는 구조와 검토 절차만 추가합니다. live `terraform import`, `apply`, `destroy`, CloudFormation stack 삭제, AWS resource mutation은 수행하지 않습니다.
...
| `legacy-rollback` | `infra/aws/production/legacy-rollback` | `production/legacy-rollback/terraform.tfstate` | EC2/SSM/nginx rollback와 legacy edge | rollback 종료 승인 전 비어 있음 |
...
5. **legacy 보존**: EC2/SSM/nginx rollback 종료가 승인되기 전에는 legacy group을 비워 둡니다. CloudFormation이 소유한 child resource는 stack이 존재하는 동안 Terraform으로 중복 소유하지 않습니다.
...
`.github/workflows/production-infra-plan.yml`은 `workflow_dispatch` 전용입니다. 선택한 group과 정확히 일치하는 `<group>-review-only` 확인 문자열이 필요하며, GitHub Environment `production-infra-plan` reviewer 승인을 통과해야 합니다.
...
runtime plan은 production 값을 누락해 secret ARN이나 runtime 설정을 제거하는 잘못된 diff를 만들지 않도록 전체 tfvars JSON이 없으면 실패합니다. workflow는 binary plan artifact를 저장하지 않고 log에 review-only plan만 남깁니다. tfvars JSON에는 secret 원문을 넣지 않고 ECS task가 참조할 Secrets Manager/SSM ARN만 넣습니다. `apply`, `destroy`, `import`, `-auto-approve` 명령은 포함하지 않습니다.

plan role에는 다음 두 경계만 허용합니다.

- 선택한 backend prefix의 state read/write와 `.tflock` 생성/삭제
- ECS, ELBv2, ECR, IAM, CloudWatch Logs/Alarms, Route53, ACM, S3 configuration, RDS, ElastiCache, EC2/SSM의 `Get*`, `List*`, `Describe*` read-only 조회
...
- CloudFormation ownership 해소 계획
- EC2 rollback 종료 또는 유지 결정
