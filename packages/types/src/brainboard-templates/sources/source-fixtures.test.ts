...
type PublicContract = {
  readonly brainboardTemplateManifest?: readonly { readonly id: string }[];
...
test("source registry exposes all 23 successful captures in manifest order", () => {
  ...
  assert.deepEqual(
    ...
    manifest
      .map(({ id }) => id)
      .filter((id) => id !== "brainboard-aws-instance-db-multiple-networks")
  );
  assert.equal(sources.length, 23);
  assert.ok(sources.every(({ captureStatus }) => captureStatus === "captured"));
...
test("presentation mappings preserve raw types and leave the blank EKS icon unresolved", () => {
  ...
  assert.deepEqual(training.nodes.filter(isPresentationNode).map(presentationIdentity), [
  ...
  assert.deepEqual(eks.nodes.filter(isPresentationNode).map(presentationIdentity), [
  ...
  const blankIcon = eks.nodes.find(
...
test("Training keeps raw source files but sanitizes only UUID metadata in workspace seeds", () => {
  ...
  for (const sourceFile of training.terraform.files) {
    ...
    assert.equal(
      sourceFile.code.split(omittedLine).length,
      2,
      `${sourceFile.fileName} exact omission`
    );
...
function verifyFixture({
  ...
  assert.deepEqual(contract.validateBrainboardTemplateSource?.(source), {
    valid: true,
    errors: []
  });
  ...
  assert.deepEqual(
    new Set(mappings.map(([, address]) => address)),
