...
- 2026-07-23: Added an API-free Dashboard My Page preview. The Settings navigation reveals an indented `마이페이지` entry, and the sidebar profile summary links to the same route. The page now presents a current-password confirmation step followed by an editable name, new-password, confirmation, and final `수정하기` layout. The UI explicitly marks server-side password verification and persistence as pending; no authentication API, DB, or shared contract changed. Web lint, typecheck, and production build pass.
