...
- 2026-07-23: Added an API-free Dashboard My Page preview. The Settings navigation now reveals an indented `마이페이지` child entry, the sidebar profile summary links to the same profile route, and the read-only personal-information form uses the authenticated user's current nickname, email, and username. Root lint and typecheck pass; the direct Web production build passes. The root Turbo build exceeded its 120-second limit before the successful direct Web build.
